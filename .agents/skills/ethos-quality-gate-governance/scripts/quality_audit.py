...
def _coverage_policy_gaps(root: Path) -> list[str]:
    ...
    if coverage_policy.get("current_hard_floor") != 95:
        gaps.append("quality_coverage_floor_not_95")
    ...
    if "fail_under = 95" not in coverage_ini:
        gaps.append("quality_coverage_ini_floor_not_95")
...
def main() -> int:
    ...
    for report in (type_report, doc_report):
