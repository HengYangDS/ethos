...
- The root `.gitleaks.toml` owns secret-scanning policy; `mise.toml` and `mise.lock` own native tool selections and platform artifact digests. `tools/ci/scripts/run-secrets-scan.sh` obtains verified project-local supply through `tools/ci/toolchain/native.py` and passes the policy explicitly. Hosted proof uses the same supply owner; neither path installs system files or trusts ambient scanner bytes.
...
- mise.toml and mise.lock select native developer/CI supply. The existing
...
- `.config/checks/github/actionlint.toml` owns GitHub workflow syntax policy;
  ...
  resolves the exact installed tool through `mise.toml` and `mise.lock`; missing
