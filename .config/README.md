...
- `.config/checks/ruff/ruff.toml` is the sole Ruff lint-policy owner. The root `ruff.toml` is a discovery adapter for IDEs, agents, and direct Ruff invocations: it contains only checkout-relative cache routing and `extend`, never a second rule set.
