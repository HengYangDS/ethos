...
Executable topology policy lives beside its interpreter in
`src/ethos/contracts/artifacts/topology.toml`. Native Python package resources
load the same declaration in source and installed execution; the working
directory cannot replace it. Runtime code and policy migrate as one versioned
unit. A missing or malformed package policy fails rather than falling back to
another checkout. Repository-declared gate configuration has a different owner
and is not changed by this binding.

Path families, gap prefixes and lifecycle classes are declarative. Origin comes
from lifecycle ownership or an existing native producer relation, never from a
suffix, formatter or tracking status. An ordinary authored JSON file remains
unclassified source; a declared Mermaid output remains a producer-owned
projection. Placement, origin, existence and proposed effect are separate facts.
Public patch and staged-index admission validate generated bytes against their
producer and preserve deletions when checking surviving native consumers.
...
Adopters do not need product-owned `adopters/<name>`, `profiles/<name>`, fixture
directories or a parallel generated-artifact carrier. Existing native producer
declarations retain source/output ownership. Runtime policy comes from the
installed package, not an adopter copy. Retirement removes or redirects native
producer relations together with their outputs and required consumers; deleting
a declaration alone cannot legitimize arbitrary retained output bytes. Preserve
selected Attestations and supporting objects still required by current consumers.
