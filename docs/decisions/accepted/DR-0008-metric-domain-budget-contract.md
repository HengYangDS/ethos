...
The current v1 program is already blocked. Its baseline, allowance, debt dates,
terminal targets, current gaps, and known replay drift are obligations that a
unit migration must preserve rather than normalize away.
...
- The first implementation slice is behavior-preserving extraction from
  `ethos.domain.prove`; it changes ownership, not enforcement.
