...
| status | product and embedded bounded reader views | `ethos-repository` | migrate-to-product | status golden output | same governance, evidence, command-surface, and projection health verdicts | embedded status used for diff |
...
| Hatchling local build | embedded Hatchling package metadata and product metadata | all Python packages | already-in-product | `uv build --out-dir build/artifacts/python --clear --no-create-gitignore` | all Python packages build wheel and sdist locally | no remote release needed |
