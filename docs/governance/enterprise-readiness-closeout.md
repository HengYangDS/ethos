...
| L4 shared command plane | governance context on command payloads | Product and adopter repositories share one bounded `status` reader and the `plan -> prove -> land -> publish` lifecycle. |
...
| L7 enterprise operability | `ethos status` hard quality floor | Product-boundary and identity failures must appear as required gaps, never hidden by a green reader verdict. |
