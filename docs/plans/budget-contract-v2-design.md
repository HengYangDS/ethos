...
Status: approved for governed implementation on 2026-07-19.
...
The current v1 state is also already blocked. Migration must preserve that fact:
it cannot reset the baseline, extend debt, raise the allowance, average-convert
LOC into another unit, or make a required gap disappear merely because the unit
changed.
