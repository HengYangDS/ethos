...
- Work only in the admitted history-residue Work Lane (`$ETHOS_WORK_LANE`) with `ETHOS_ACTOR=agent:openai:thread:root`.
...
- Create: `packages/ethos/src/ethos/adapters/store/state/schema.py`
...
- Local source: `$ETHOS_ACCEPTED_ROOT/.ethos/state/residue-snapshots/`
- Operator archive: `$ETHOS_RECOVERY_ROOT/history-residue-closeout-20260719/`
...
- [ ] Dry-run maintenance against `$ETHOS_ACCEPTED_ROOT` and record the inventory digest and exact counts.
...
- Delete: `packages/ethos/src/ethos/adapters/admission/control/verifier.py`
- Delete or reduce: `extensions/independent-verification/tests/`
- Modify: `packages/ethos/src/ethos/adapters/admission/control/replacement.py`
- Modify: `packages/ethos/src/ethos/adapters/admission/evidence/external.py`
- Keep: `packages/ethos/src/ethos/repository/evidence/attestation.py` as the distinct
  semantic-claim receipt boundary
...
- [ ] Keep semantic attestation and signed independent verification as distinct typed
  trust boundaries while deleting the unused identity/hosted file-report wrapper.
- [ ] Remove all three bundled verifier executables and stale product-shipping claims/docs while retaining external operator guidance.
...
**Interfaces:** Public command shape, schema validation, and 100% coverage remain
stable; closeout gap codes may be strengthened when required to fail closed on
unavailable control diffs, candidate-HEAD drift, or candidate-local bootstrap
artifacts.
...
- [ ] Run `status`, `plan --changed`, and HEAD-bound executed proof, then complete and archive the carrier and commit the archived change.
- [ ] On the archived HEAD, rerun proof, land to candidate, close out accepted root, and run local publish readiness; report remote push and hosted observations as deferred.
