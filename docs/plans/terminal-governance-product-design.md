...
The September 16 supply work has two dependent batches under the existing Change.
Official release observations, native lock dry-runs and exact image digests are
retained in `build/evidence/quality/commit-integrity/`. First-batch source
`1c27947f2130722e9bc8d2befb44309ce3c37428`, tree
`a9d0d766ac010de76c126c70a073cf26c6d65c05`, passed all 35 full-proof gates,
...
suffix list. Selected committed shell owners admit their executables; unselected
scripts and mutable checkout tampering do not. After 76 admission regressions,
321 related consumers and full proof, the installed repair admitted the exact CI
patch previously blocked for uv. No false declaration or hook bypass was used.

The second batch selects uv 0.12.15, setup-uv 10.1.0 at its immutable commit,
Node 26.8.2 plus LTS 24.21.0, official archive checksums and the exact uv/Python
image digest. Both hosted projections match their native templates; 112 consumer
tests and Ruff/config/shell/CI checks pass. In the pinned Linux ARM64 container,
...
The same adopter's native refresh then encounters a divergent existing proposal;
its receipt gives the same non-progressing probe. Current signature-repair
provenance is not generic rebase provenance. The existing refresh/publication
owners must preserve old review objects, prove the bounded replacement relation,
...
| Quality and full-cycle speed | Complete local proof reached 670.898 seconds; 600 seconds remains a direction. The official type gate resolves package `.` to `src`, so its PASS does not prove test/tool typing; a wider direct check exposed unresolved diagnostics. | Existing quality owners and P5: close semantic size, docstring/type/format scope, distinguishing failures, input-complete evidence reuse and actual first-failure/closeout measurements without narrowing obligations. |
...
| Local and independent remote workflow | Historical identity correction and both peer replacements are delivered; no permanent single-author restriction applies. Release continuity is delivered with attributed adopter acceptance; current hosted completion remains separately observed. | Existing publication/commit-policy owners: exact local/single/multi-peer OIDs, hosted CI, reports and signature observations; developer proposal retirement does not wait for main. |
...
| `codex-contracts-land-test-refactor` | All ten unique commits and 45 changed paths are classified in the [priority retirement disposition](#codex-topic-priority-retirement). Current counterparts passed 17 focused cases. Public terminal receipt `b245cc42` and exact postchecks prove worktree, administration and local ref retirement; disposition acceptance remains pending. |
...
The user's September 11 priority supersedes waiting for hosted-proof closeout
before retiring this topic. Source `c4ec2ebbc05beb11df15b1b8528c182402c0effe`
contained ten commits beyond accepted history, touching 45 paths. It was clean,
with nine generated bytecode files and one dangling `.venv` symlink among ignored
content. The exact absolute target is bound by the local retirement receipt;
its basename is `codex-contracts-land-test-refactor`. Neither its parent,
sibling lanes nor the symlink target is a deletion target.

| Historical meaning | Disposition and current owner |
| --- | --- |
| Exact primitive CAS, source/target assertions, stale receipt rejection, compensation and hook intent | Retain through `GitEffect`, `TransitionPlan`, Git-effect admission/execution and accepted promotion. Supersede the private `role.transition` command, edge model, request store and command-specific recovery. Arbitrary profile-edge execution is not a current capability claim. |
| Source/package schema agreement and strict policy | Retain through the current schema loader and package acceptance owners. Do not resurrect the retired transition schema. Complete package conformance remains separately required. |
| Archive proof after Lease retirement, exact repository/object/policy and conflicting-proof rejection | Current `proof_admission` and `proof_for_repository_transition` cover these obligations without a persistent repository Commitment. Focused positive and negative counterparts pass. |
| One immutable Attestation snapshot per observation | Current archive transition and Git-effect validation accept one selected set; obsolete Commitment-rebind projection is removed. This is bounded observation reuse, not indefinite proof caching. |
| Materialize Commitment before OpenSpec | Superseded by official OpenSpec-only intent and transient compilation. The historical bootstrap file and Lease binding must not return. |
| Cheap common-state admission | Current common-directory state schema replaces the retired per-worktree migration planner; it does not build a whole runtime inventory to locate Lease state. |
| Cheap preflight fixtures | Preserve the principle of testing required state rather than repeatedly executing unrelated effects. The old rollover consumer and its fake archive/Lease-rebind fixture no longer exist; keep real archive integration coverage rather than restore that implementation. |
| Dynamic worker balancing | Current test gate already selects work stealing, with an executable argument regression. No additional workers are enabled. |
| Shared immutable test supply | The old global temporary wheel cache and locking wrapper are superseded by minimal non-runtime fixtures sharing dependency supply. The current child-source-binding correction is focused-green but unaccepted; broader runtime-test cost remains open. |
| Official Change intent and progress | Preserve the distinct obligations above and the exact raw delta as temporary recovery evidence. Old checkboxes and its unpassed proof never become current acceptance. |

Every changed path is assigned exactly once in
`codex-topic-semantic-disposition.json`, SHA256
`e8407eb4f5bf6a1085f8fb0db702d6a4b4aa624f26c7b8aa9bad71d62f631a39`,
under the existing ignored evidence root. It binds original and accepted-owner
hashes, ten commit identities and literal ignored residue. Seventeen current
counterpart tests passed in 45.78 seconds, with their isolated scratch removed.
The exact unique commit range is recoverable from the verified 41,818-byte
`codex-topic-unique-history.bundle`, SHA256
`ef69714e401a6137f9e450a7ea2f7f620f3950f58a17a8987c4a19539efa52dd`,
using the accepted-history base. This is not a retained runtime, checkout or
new branch. Retire that recovery bundle after this disposition is accepted and
no consumer requires the raw delta; do not retain it indefinitely by default.

Current-runtime derivation and no-effect preflight pass from the accepted
control root, despite the old checkout's obsolete gate schema. Receipt
`ab0c4a8b99eb0c9313e39429b5cf87c65fdf6bc5e1ad9cb142c0a367d7ac68c8`
binds 4,238 literal entries, exact index/ref/accepted coordinates, actor and
absence of a Lease. At 09:46 +08:00 the public receipt-bound operation returned
`pass`, `retired`, with both planned effects complete and no remaining effects.
Terminal receipt
`b245cc42089f0e5700a70d11c6ca2a316a83f1f72548c3abe49ff3d3ed21b3e2`
is stored by the existing Git-common retirement owner. Independent postchecks
confirmed absent target path, Git administration, registration and local ref;
all other branch/tag/tracking refs, three Lease rows, runtime selector and five
remaining roots' HEAD/index/dirty bytes were unchanged. The dangling symlink
was unlinked without following or creating its target. No hook bypass, new lane,
replacement branch or remote mutation was used. The checked readback is
`codex-topic-retirement-poststate.json`, SHA256
`24bc563527fa571348d1a0e6d6bbfe7d80f0e0114d79a0dd26676fd72d0c420b`.
Physical retirement is complete; acceptance of this disposition and the pending
CI fixture correction remain distinct, unfinished source-delivery obligations.
...
The source evidence is the owner-provided `disk-cleanup-20260828-1243` package;
its exact local path is retained in the ignored input receipt. Its inventory
...
#### Hosted Failure Recovery Checkpoint — September 11, 2026

At 09:33 +08:00 the accepted source remains `10d328944`; the owned authoring
lane is `work/20260907-missing-lease-reconciliation` at `96286e581`, with
uncommitted fixture corrections. Fresh status and exact-path prewrite pass;
Lease generation 8 matches the invocation actor. No new full proof, accepted
activation, hosted success or lane retirement is claimed.

| Boundary | Demonstrated cause, repair and remaining evidence |
| --- | --- |
| Three hosted archive failures | Collection-time UTC date disagreed with execution-time date. The original cases pass on the same day and fail across midnight. Explicit-clock scenarios now pass with real Git preservation on both dates; product date semantics are unchanged. |
| Runtime fixture source identity | A POSIX trampoline retained the parent prefix and ignored the fixture source projection. The native minimal prefix and selected-interpreter fixture binding pass isolated child-origin and lifecycle checks on both available Python supplies. This is not a native-startup speed claim. |
| Native hardlink retirement failure | Historical output hid `process_failure`; no exact cause is established. Complete JSON assertion diagnostics are added. Prior successful serial observations do not prove repair, and unknown liveness remains blocking. |
| Dependent gate failures | Coverage-floor and generated-artifacts did not execute after the failed test gate; they are not separately measured regressions. A new exact-source full proof remains required. |
| Scratch cleanup | Hosted normal failure-exit cleanup was observed. SIGKILL orphan recurrence, descendant-aware recovery and actual long-run retention remain open under the resource checkpoint. |

The two focused Python runs each passed 74 tests (69.78 and 47.72 seconds),
including source/prefix identity, archive collision cases and the two lifecycle
regressions. Ruff, format and explicit changed-file Ty checks pass. Both owned
scratch roots were removed. Commands, source hashes and results are in
...
Change, not dropped or recreated as a parallel active carrier. The preservation
and withdrawal receipts remain recovery evidence until accepted results replace
their consumers.

The 09:33 lane inventory contained the current work lane, the dirty historical
`20260811-repository-transition-model`, the clean transferred
`20260908-accepted-carrier-signature-repair`, and the separate old
`codex/contracts-land-test-refactor` worktree. The two historical work-lane
Leases are expired, not missing; prior holder transfer is not current write
admission. The user's later priority retired the codex topic as recorded in the
[exact disposition](#codex-topic-priority-retirement). Next prioritize
exact-equal/absorbed retirement, then extract remaining useful deltas
into the current model and retire their origins. Do not revive obsolete models
merely to close their lanes or postpone easy retirement behind the global audit.

Next within this atom: bounded native failure diagnosis, focused admission and
static checks, signed source freeze, then exact-HEAD full proof with at most two
workers. Keep archive/accepted/runtime and each peer's publication/CI readback
separate. Continue the existing global stages and resource checkpoint afterward;
no second roadmap or wider product redesign is introduced by this fixture repair.
...
### Exact Proof Failure Disposition — September 11

The exact `621542420` proof is blocked: 5 failed, 2906 passed and 1 skipped in
1283.83 seconds; total proof elapsed was 1320.81 seconds. Combined coverage was
95.2094%, not a passing coverage gate: dependent gates did not execute. Original
archive and hardlink cases passed this attempt, which does not establish the
historical native-observer cause. The owned pytest root was removed on exit.

Two deterministic failures were corrected: tracked plan host paths now use
portable locators, and the fixture budget separately verifies one exact native
interpreter plus fewer than one million overhead bytes and at most five files.
The two boundary modules passed 13 tests before the portable same-file check was
refined. Native interpreter size is supply, not duplicated fixture overhead;
product/test ELOC and coverage floors are unchanged.

The three activation query timeouts passed a narrow covered replay in 5.51
seconds, with child queries between 0.49 and 0.69 seconds. This is not a latency
repair. A deterministic public-command regression then proved that activation
discarded the reader's exact timeout evidence after rollback and recommended
reinstallation. The existing process-error carrier now retains that observation,
and public continuation requests status rather than repeating effects. Legacy
and absent-state cases both failed on missing evidence before the repair and
passed afterward, with actual database, selector and Git-configuration rollback.

The final affected suite passed 164 tests on Python 3.13 in 99.57 seconds;
activation and fixture-boundary checks passed 73 tests on Python 3.14 in 48.98
seconds. Both owned temporary roots were removed. Ruff, explicit changed-file Ty,
native Markdown validation, codespell and strict OpenSpec validation pass. The
public host-observation path passed python-types, module-layout, docstrings and
source-budget; it is not repository proof. Product/test ELOC measured
40081/40686 against their separate 45000 limits.

Focused and exact-proof receipts remain in the existing ignored
`build/evidence/quality/evidence-retirement` root. Next freeze this correction
and investigate the bounded native startup boundary before another full proof.
Accepted remains `10d328944`; no
new runtime, archive or hosted success is claimed. The AIGW authored/generated
