...
- `alternate mechanism corpus` informed capability-local profiles, direct routing, reuse stance,
  dynamic facets, live-spec diff guards, and archive normalization.
- `reference adopter workspace` informed the portable campaign pattern: a read-only
...
- After merging current `dev`, the existing closeout package test failed on
  ...
  entries and a reference-adopter shadow refresh package for the current product HEAD.
...
`reference-adopter` pointed at an older product HEAD. The Work Lane refreshed both
tracked evidence files through the ETHOS command plane:

```text
uv run --package ethos ethos parity shadow --adopter generic --target <worktree> --execute --write-evidence --json
uv run --package ethos ethos parity shadow --adopter reference-adopter --target <adopter-repo> --execute --write-evidence --json
```
