...
plane interpret the reference adopter without relying on the embedded ETHOS
...
- The tracked reference-adopter parity evidence uses the profile evidence root
  with a neutral `<adopter-id>-shadow.json` filename.
...
- Repo-local skill activation records with `path_globs` are normalized to the
  ...
  routing contract while supporting reference-adopter-style activation metadata.
...
```bash
uv run --package ethos ethos quality command-surface --root <adopter-repo> --json
uv run --package ethos ethos prove --root <adopter-repo> --json
...
- External command-surface against reference-adopter: `ok=true`, no required gaps.
- External proof against reference-adopter: `ok=true`, adopter audit mode, no
  required gaps.
- External report against reference-adopter: `ok=true`, score `7 / 7`, no required
  gaps.
- External playbook routing against reference-adopter: `ok=true`, repo-local skills
  with path glob activation were recognized.
- Real shadow parity against reference-adopter: `ok=true`, `state=matched`,
  required gaps empty across nine public commands.
- Evidence-driven parity gaps for reference-adopter: `ok=true`, `gap_count=0`,
  required gaps empty.
