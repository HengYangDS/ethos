...
function standaloneReader() {
  ...
  function select() {
    ...
    if (url.hash.startsWith("#atlas=")) {
      const state = new URLSearchParams(url.hash.slice(7));
      return {
        page: state.get("page") || "index.html",
        query: state.get("query") || "?theme=light",
        hash: state.get("hash") || "",
      };
    }
    const page = url.searchParams.get("page") || "index.html";
    url.searchParams.delete("page");
    return { page, query: url.search || "?theme=light", hash: url.hash };
  }
  function virtualUrl() {
    const state = select(),
  ...
  function writeState(state, replace) {
    const url = new URL(location.href);
    url.hash = "atlas=" + new URLSearchParams(state).toString();
    try {
      history[replace ? "replaceState" : "pushState"](null, "", url);
    } catch (_) {
      if (replace) location.replace(url.hash);
      else location.hash = url.hash;
    }
  }
  window.__atlasLocation = {};
  for (const key of ["href", "pathname", "search", "hash", "origin"])
    Object.defineProperty(window.__atlasLocation, key, { get: () => virtualUrl()[key] });
  window.__atlasHistory = function (url, replace) {
    const target = new URL(url, virtualUrl());
    const page = target.searchParams.get("page") || select().page;
    target.searchParams.delete("page");
    writeState({ page, query: target.search, hash: target.hash }, replace);
  };
  function bridge() {
    const parentWindow = window.parent;
    window.__atlasLocation = {};
    for (const key of ["href", "pathname", "search", "hash", "origin"])
      Object.defineProperty(window.__atlasLocation, key, {
        get: () => parentWindow.__atlasLocation[key],
      });
    window.__atlasHistory = {
      replaceState: function (state, title, url) {
        parentWindow.__atlasHistory(url, true);
      },
      pushState: function (state, title, url) {
        parentWindow.__atlasHistory(url, false);
      },
    };
    document.addEventListener(
      "click",
      function (event) {
        const anchor = event.target.closest && event.target.closest("a[data-atlas-page]");
        if (!anchor) return;
        if (
          event.button !== 0 ||
          event.metaKey ||
        ...
        parentWindow.__atlasNavigate(
          anchor.dataset.atlasPage,
          anchor.dataset.atlasQuery,
          anchor.dataset.atlasHash,
        );
      },
      true,
    );
  ...
  function hydrate(page) {
    const html = data.pages[page].pieces
      .map((x) => (typeof x === "number" ? data.blocks[x] : x))
      .join("");
    ...
    for (const anchor of doc.querySelectorAll("a[href]")) {
      ...
      if (!hasPage(name)) throw new Error("Missing embedded page: " + name);
      anchor.dataset.atlasPage = name;
      anchor.dataset.atlasQuery = target.search;
      anchor.dataset.atlasHash = target.hash;
      const link = new URL(location.href);
      link.hash =
        "atlas=" +
        new URLSearchParams({ page: name, query: target.search, hash: target.hash }).toString();
    ...
    start.textContent = "(" + bridge.toString() + ")();";
    doc.head.prepend(start);
  ...
  function render() {
    ...
    if (!hasPage(page)) {
      error.hidden = false;
      error.textContent =
        "This page is not included. Remove the page selection to open the overview.";
      frame.hidden = true;
      return;
    }
    error.hidden = true;
    frame.hidden = false;
    if (page === active) {
      frame.contentWindow?.dispatchEvent(new Event("hashchange"));
      return;
    }
    active = page;
    frame.srcdoc = hydrate(page);
  }
  window.__atlasNavigate = function (page, search, hash) {
    if (!hasPage(page)) return;
    writeState({ page, query: search || "?theme=light", hash: hash || "" }, false);
    if (page === active) active = null;
    render();
  };
  ...
  try {
    ...
    error.hidden = false;
    error.textContent = "The embedded atlas could not open: " + e.message;
    frame.hidden = true;
  }
}
