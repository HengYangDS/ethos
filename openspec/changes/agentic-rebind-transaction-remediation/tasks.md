# Tasks

- [x] 1.1 Add a RED status regression for an authenticated archive effect whose
  carrier has moved outside the former active scope glob.
- [x] 1.2 Authorize exact archive-effect paths in current-generation attribution
  while preserving tampered or missing attestation failures.
- [x] 1.3 Run the post-archive status/plan/prove focused matrix and commit the
  smallest signed atom.
- [x] 2.1 Add RED derive tests for exact target discovery, ambiguity, absent
  target, and package-only CLI output.
- [x] 2.2 Persist a digest-bound derive receipt over the existing exact request
  contract.
- [x] 2.3 Add receipt-bound dry-run/apply and reject HEAD, index, overlay, Lease,
  carrier, or target drift.
- [x] 3.1 Project `commitment_rebind_required`, dangling target OID, typed
  mismatches, partial effects, and one public remediation command.
- [x] 3.2 Distinguish missing invocation actor from holder mismatch and correct
  editor-root remediation for a valid Lease.
- [x] 3.3 Prove checkout and source-independent package-only parity.
- [x] 4.1 Run focused tests, OpenSpec strict validation, Ruff, and config quality.
- [x] 4.2 Create signed atomic commits and execute one exact-HEAD full proof at
  the governed coverage floor.
- [x] 4.3 Verify exact-HEAD proof, archive readiness, and the governed closeout
  command sequence before executing the receipt-backed lifecycle effects.
