...
nodes grow. The predecessor foundation observation was blocked with 17 required
gaps. The successor candidate later projects campaign-terminal growth as
advisory while retaining terminal and debt facts. Its declared baseline also
replays 283 ELOC lower under today's v1 algorithm because metric semantics
...
5. The first implementation step is behavior-preserving extraction to
   ...
   forwarder. The current v1 `SourceBudgetTaxonomy` and `campaign_terminal`
   projection remain inputs to that extraction; neither is Budget v2 completion.
...
HEAD-bound executed default proof. Controlled inputs must retain the same
taxonomy, policy, campaign, debt, command-state, required-gap, and advisory-gap
semantics. The predecessor 17-gap observation remains historical evidence; the
successor's advisory projection is not settlement or v2 cutover.
