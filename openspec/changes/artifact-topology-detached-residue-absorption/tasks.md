...
- [x] 1.1 Capture the detached worktree registration, reflog, exact HEAD, dirty
  paths, file hashes, patch digest, owner/session state, and path occupancy.
- [x] 1.2 Reconstruct the historical branch at the exact detached HEAD without
  changing index or working bytes.
- [x] 1.3 Re-observe the normalized lane as linked, dirty, missing-lease,
  missing-Claim, accepted-ancestor, and unpreserved.

## 2. Semantic authority carrier

- [x] 2.1 Map every historical concern to absorbed current behavior or an
  explicit rejection.
- [x] 2.2 Add the target-specific design, implementation plan, Claim, Chronicle,
  OpenSpec change, and delta scenario.
