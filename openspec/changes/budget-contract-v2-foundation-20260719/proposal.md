...
needs a metric-domain contract before changing enforcement, and the existing v1
blocked state must remain visible throughout migration.
...
- Extract the existing v1 source-budget domain behavior from `domain/prove.py`
  ...
  gate membership, or required-gap semantics.
...
registry provider, and focused tests. This Change does not alter v1 thresholds,
debt dates, source inventory, proof scope, or enforcement.
