...
ETHOS SHALL preserve the current v1 source-budget baseline, thresholds, debt
lifecycle, inventory, and required gaps while migrating to Budget Contract v2.
The migration SHALL introduce a typed carrier inventory and versioned,
...
- **GIVEN** the v1 source-budget command is blocked at a stable HEAD
- **WHEN** its domain implementation moves from `ethos.domain.prove` to
  `ethos.domain.source_budget.core`
- **THEN** command JSON, exit status, baseline identity, current metrics,
  inventory digest, debt lifecycle, and required-gap semantics SHALL remain
  equivalent
