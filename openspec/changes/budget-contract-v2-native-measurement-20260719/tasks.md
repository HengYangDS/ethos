...
- [x] 4.1 Implement canonical-runtime admission, cached provider conformance self-test, strict UTF-8/newline normalization, exact provider dispatch, Python/footprint/Jinja parsing, and Shell v3 in declaration-only `measurement/native/shell/__init__.py` plus bounded `core.py` and `grammar.py`; include stable resource-exhaustion gaps and descriptor cleanup on every exit.
