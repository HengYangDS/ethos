...
The workstation control plane can issue a read-only ownerless closeout
...
effect fence, does not validate every WCP response binding, can omit the
...
- Add a bounded WCP adapter that validates every security-bearing response
  field against the exact decision bytes, observation, executor, accepted
  branch, and accepted head supplied by ETHOS.
- Add a Git-common-directory target fence acquired through SQLite CAS after WCP
  admission and before effect; lease acquisition for the same lane fails while
  the fence exists.
...
- Bind completion receipts to executor, WCP decision digest, accepted head,
  coordination binding, target binding, and postcondition digest.
...
- `repository-governance`: subject=ownerless-closeout-effect-fence;
  ...
  facet:authority=source,test,contract,evidence,wcp-admission
