...
- [x] 1.1 Add a single repository-owned Python/uv runtime bootstrap with checkout-bound `build/runtime/venv` and explicit host-or-CI cache behavior.
- [x] 1.2 Route the Work Lane runner, Python owner scripts, local fallback CI, installed Git hooks, and the named provider projections through the bootstrap or explicit bounded interpreter path.
- [x] 1.3 Remove active root `.venv` fallback and bare producer `uv run` paths from product-owned executable surfaces.
