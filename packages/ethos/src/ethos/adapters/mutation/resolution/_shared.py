...
def artifact_roots(root: Path) -> tuple[Path, ...]:
    ...
    control_root = accepted_control_root(root)
    registered = tuple(
        Path(record["worktree"]).resolve() / LEGACY_ARTIFACT_ROOT
    ...
    candidates = (
        ...
        *registered,
        root.resolve() / LEGACY_ARTIFACT_ROOT,
    )
...
def canonical_record_path(root: Path, path: Path) -> bool:
    ...
    try:
        return path.resolve().is_relative_to(records_artifact_root(root).resolve())
    except ValueError:
        return False
...
def canonical_package_path(artifact_root: Path, decision_id: str) -> Path | None:
    ...
    return candidate if candidate.resolve().is_relative_to(artifact_root.resolve()) else None
