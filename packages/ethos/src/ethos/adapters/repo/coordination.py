...
def coordination_state(
    *, current_role: str, current_path_scope: tuple[str, ...], current_scope_state: str,
    ...
    if "unknown" in states:
        return "unknown"
    return "overlap" if scopes_overlap(current_path_scope, foreign_path_scope) else "disjoint"
...
def foreign_work_lane(  # noqa: PLR0913, RUF100 - exact bound-state dimensions
    ...
    committed, committed_state = branch_path_scope(
        root, branch=worktree["branch"], candidate_branch=candidate_branch
    )
...
def coordination_gaps(
    ...
    required = ["coordination_gap:current_scope_unknown"] if (
        current_role == ROLE_WORK_LANE and current_scope_state == "unknown"
    ) else []
...
def closeout_disposition(
    *, lease_state: str, claim_binding: str, relation_to_accepted: str, dirty: bool
    ...
    return {
        "descendant_of_accepted": "unlanded",
        "diverged_from_accepted": "diverged",
    }.get(relation_to_accepted, "none")
...
def coordination_next_action(  # noqa: PLR0911, PLR0913, RUF100 - exact public decision dimensions
