...
def landing_readiness(
    ...
    if role != ROLE_WORK_LANE:
        state, gaps, action = "not_work_lane", [], "start or enter a Work Lane before landing"
    elif not candidate.get("exists"):
        state, gaps, action = (
            "blocked", ["candidate_branch_missing"],
            "create or repair the configured candidate branch",
        )
    elif not candidate.get("worktree_exists"):
        state, gaps, action = (
            "blocked", ["candidate_worktree_missing"],
        ...
        state, gaps, action = (
            "candidate_base_stale", ["candidate_base_stale"],
            "ethos lane refresh-base --apply --authorize "
            f"--expect-head {head or '<head>'} --json",
        )
    else:
        state, gaps, action = "candidate_base_current", [], "ethos land --json"
    return {
        "kind": "landing_readiness", "state": state, "branch": branch, "head": head,
        "candidate_branch": candidate_branch, "candidate_head": candidate_head,
        "required_gaps": gaps, "next_action": action,
    }
...
def workspace_status(root: Path, *, include_foreign_path_scope: bool = True) -> dict[str, object]:
    ...
    branch, head = current_branch(root), _safe_ref(root, "HEAD")
    ...
    current_scope, scope_state = (
        ...
        if include_foreign_path_scope else ((), "deferred")
    )
    foreign = _foreign_work_lanes(
        worktrees, current_path=repo, current_role=role, current_path_scope=current_scope,
        current_scope_state=scope_state, accepted_branch=policy.accepted_branch,
        candidate_branch=policy.candidate_branch, lease_by_branch=leases, root=repo,
        include_path_scope=include_foreign_path_scope,
    )
    required, advisory = coordination_gaps(
        foreign, current_role=role, current_scope_state=scope_state
    )
    ...
    coordination = coordination_package(
        foreign, required_gaps=required, advisory_gaps=advisory,
        unbound_work_lane_refs=unbound_refs,
    )
    support = closeout_support(
        branch=branch, role=role, dirty=bool(paths), candidate=candidate,
        lease_by_branch=leases, coordination_required_gaps=required,
    )
    landing = landing_readiness(repo, branch=branch, role=role, candidate=candidate)
    return {
        "root": str(root), "branch": branch, "head": head, "dirty": bool(paths),
        "changed_paths": list(paths), "dirty_provenance": provenance, "role": role,
        "role_policy": policy.as_status_policy(), "runtime_binding": runtime_binding(repo),
        "landing_readiness": landing, "candidate": candidate, "worktrees": worktrees,
        "branch_bindings": bindings, "foreign_work_lanes": foreign,
        "coordination_gaps": [*required, *advisory], "coordination": coordination,
        "closeout_support": support,
        "stage_gates": _stage_gates(
            branch=branch, role=role, closeout_support=support, landing_readiness=landing
        ),
...
def _stage_gates(
    *, branch: str, role: str, closeout_support: Mapping[str, object],
    ...
    commands = ["ethos lane prewrite <path>"] if authoring else []
    if stale:
        commands.append(
            str(landing_readiness.get("next_action") or "ethos lane refresh-base --json")
        )
    elif integration:
        commands.append("ethos land --json")
    if not commands:
        commands.append("ethos lane start <name>")
    if not authoring:
        blocked, owner = "authoring", branch if is_work_lane else ""
    elif not integration:
        blocked, owner = "candidate_integration", (
            str(landing_readiness.get("candidate_branch") or branch) if stale else branch
        )
    else:
        blocked, owner = "accepted_closeout", str(closeout_support.get("target_branch") or "")
    return {
        "authoring_allowed": authoring, "integration_allowed": integration,
        "accepted_closeout_allowed": False, "blocked_stage": blocked,
        "blocker_owner": owner, "recommended_next_command": commands[-1],
...
def _non_git_status(root: Path) -> dict[str, object]:
    ...
    candidate: dict[str, object] = {
        "branch": policy.candidate_branch, "exists": False, "head": "",
        "worktree_exists": False, "worktree_path": "", "worktree_binding": "absent",
    }
    landing = {
        "kind": "landing_readiness", "state": "not_work_lane", "branch": "untracked",
        "head": "", "candidate_branch": policy.candidate_branch, "candidate_head": "",
        "required_gaps": ["git_repository_missing"],
        "next_action": "enter a Git-backed Work Lane before landing",
    }
    support = {
        "supported": False, "branch": "", "target_branch": policy.candidate_branch,
        "target_path": "", "operation": "", "holder_ref": "", "lease_id": "",
        "lease_epoch": 0, "claim_id": "", "claim_binding": "unbound",
        "required_gaps": ["protected_root_mutation", "git_repository_missing"],
    }
    return {
        "root": str(root), "branch": "untracked", "dirty": False, "changed_paths": [],
        "dirty_provenance": {
            "dirty": False, "state": "non_git", "entries": [], "summary": {},
            "temporary_probes": {"count": 0, "paths": [], "truncated": False},
        },
        "role": "other", "role_policy": policy.as_status_policy(),
        "runtime_binding": runtime_binding(root), "landing_readiness": landing,
        "candidate": candidate, "worktrees": [],
        "branch_bindings": branch_bindings(root, [], candidate, policy=policy, lease_by_branch={}),
        "foreign_work_lanes": [], "coordination_gaps": [],
        "coordination": coordination_package([], required_gaps=[], advisory_gaps=[]),
        "closeout_support": support,
        "stage_gates": _stage_gates(
            branch="untracked", role="other", closeout_support=support,
...
def worktree_records(
    ...
    return [
        _normalize_worktree(
            ...
            current_path=current_path, policy=policy,
        )
        for block in blocks
    ]
...
def _normalize_worktree(
    ...
    return {
        "path": path, "head": entry.get("HEAD", ""), "branch": branch or "detached",
        "role": policy.role_for_branch(branch),
        "worktree_binding": worktree_binding(path, current_path=current_path),
    }


def _foreign_work_lanes(  # noqa: PLR0913, RUF100 - exact bound-state dimensions
    worktrees: list[dict[str, str]], *, current_path: Path, current_role: str,
    current_path_scope: tuple[str, ...], current_scope_state: str, accepted_branch: str,
    candidate_branch: str, lease_by_branch: dict[str, dict[str, object]], root: Path,
    include_path_scope: bool,
) -> list[dict[str, object]]:
    foreign: list[dict[str, object]] = []
    for worktree in worktrees:
        if worktree["role"] != ROLE_WORK_LANE or Path(worktree["path"]).resolve() == current_path:
            continue
        branch, lease = worktree["branch"], lease_by_branch.get(worktree["branch"], {})
        ...
        if not include_path_scope:
            ...
            continue
        foreign.append(foreign_work_lane(
            worktree, current_role=current_role, current_path_scope=current_path_scope,
            current_scope_state=current_scope_state, candidate_branch=candidate_branch, root=root,
            lease=lease, claim_id=claim_id,
...
def _worktree_dirty_paths(worktree: dict[str, str]) -> tuple[str, ...]:
    return (
        ()
        if worktree.get("worktree_binding") == "missing"
        else changed_paths(Path(worktree["path"]))
    )
...
def _candidate_status(
    ...
    binding = worktree.get("worktree_binding", "absent")
    ...
    return {
        "branch": policy.candidate_branch, "exists": bool(head), "head": head,
        "worktree_exists": binding in {"current", "linked"}, "worktree_path": path,
        "worktree_binding": binding, "behind_accepted": behind,
    }
# fmt: on
