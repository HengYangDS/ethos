...
def acquire_lease(db_path: Path, *, subject: str, holder_ref: str, ttl_seconds: int = 86_400, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    with closing(sqlite3.connect(db_path)) as connection:
        ...
        connection.commit()
    return {"id": lease_id, "subject": subject, "expires_at": expires_at.isoformat(), **lease_contract_fields(normalized_payload), "payload": normalized_payload}
...
def renew_lease(  # noqa: PLR0913, RUF100 - exact request envelope preserves bound state dimensions
    ...
    return _refresh_lease(db_path, subject=subject, holder_ref=holder_ref, expected_lease_id=expected_lease_id, expected_epoch=expected_epoch, expected_head=expected_head, ttl_seconds=ttl_seconds, require_expired=False)
...
def offer_lease_handoff(  # noqa: PLR0913, RUF100 - exact request envelope preserves bound state dimensions
    db_path: Path, *, subject: str, holder_ref: str, expected_lease_id: str, expected_epoch: int, target_holder_ref: str, expected_head: str
) -> dict[str, Any]:
    ...
    with closing(sqlite3.connect(db_path)) as connection:
        connection.execute("pragma foreign_keys = on")
        connection.execute("begin immediate")
        row, payload = expected_current_lease(connection, subject=subject, holder_ref=holder_ref, expected_lease_id=expected_lease_id, expected_epoch=expected_epoch, expected_head=expected_head, require_expired=False)
        ...
        _update_lease_row(connection, lease_id=str(row[0]), owner=str(row[2]), expires_at=str(row[3]), payload=payload)
        connection.commit()
    return {"offer_id": offer_id, "subject": subject, "holder_ref": holder_ref, "target_holder_ref": target_holder_ref, "lease_id": expected_lease_id, "epoch": expected_epoch, "expected_head": expected_head, "state": "offered"}


def accept_lease_handoff(  # noqa: PLR0913, RUF100 - exact request envelope preserves bound state dimensions
    db_path: Path, *, subject: str, target_holder_ref: str, offer_id: str, expected_lease_id: str, expected_epoch: int, expected_head: str, holder_quiesced: bool, ttl_seconds: int = 86_400
) -> dict[str, Any]:
    ...
    with closing(sqlite3.connect(db_path)) as connection:
        ...
        _update_lease_row(connection, lease_id=str(row[0]), owner=target_holder_ref, expires_at=expires_at.isoformat(), payload=payload)
        connection.commit()
    return _lease_record(lease_id=str(row[0]), subject=subject, owner=target_holder_ref, expires_at=expires_at.isoformat(), payload=payload)


def advance_lease_head(  # noqa: PLR0913, RUF100 - exact request envelope preserves bound state dimensions
    db_path: Path, *, subject: str, holder_ref: str, expected_lease_id: str, expected_epoch: int, old_head: str, new_head: str
) -> dict[str, Any]:
    ...
    with closing(sqlite3.connect(db_path)) as connection:
        connection.execute("pragma foreign_keys = on")
        connection.execute("begin immediate")
        row, payload = expected_current_lease(connection, subject=subject, holder_ref=holder_ref, expected_lease_id=expected_lease_id, expected_epoch=expected_epoch, expected_head=old_head, require_expired=False)
        payload["expected_head"] = new_head
        payload["head_observed_at"] = datetime.now(UTC).isoformat()
        _update_lease_row(connection, lease_id=str(row[0]), owner=holder_ref, expires_at=str(row[3]), payload=payload)
        connection.commit()
    return _lease_record(lease_id=str(row[0]), subject=subject, owner=holder_ref, expires_at=str(row[3]), payload=payload)


def _refresh_lease(  # noqa: PLR0913, RUF100 - exact request envelope preserves bound state dimensions
    db_path: Path, *, subject: str, holder_ref: str, expected_lease_id: str, expected_epoch: int, expected_head: str, ttl_seconds: int, require_expired: bool
) -> dict[str, Any]:
    HolderRef.parse(holder_ref)
    initialize_state(db_path)
    now = datetime.now(UTC)
    ...
    with closing(sqlite3.connect(db_path)) as connection:
        connection.execute("pragma foreign_keys = on")
        connection.execute("begin immediate")
        row, payload = expected_current_lease(connection, subject=subject, holder_ref=holder_ref, expected_lease_id=expected_lease_id, expected_epoch=expected_epoch, expected_head=expected_head, require_expired=require_expired)
        payload["renewed_at"] = now.isoformat()
        _update_lease_row(connection, lease_id=str(row[0]), owner=holder_ref, expires_at=expires_at.isoformat(), payload=payload)
        connection.commit()
    return _lease_record(lease_id=str(row[0]), subject=subject, owner=holder_ref, expires_at=expires_at.isoformat(), payload=payload)


def expected_current_lease(  # noqa: PLR0913, RUF100 - exact request envelope preserves bound state dimensions
    connection: sqlite3.Connection, *, subject: str, holder_ref: str, expected_lease_id: str, expected_epoch: int, expected_head: str, require_expired: bool
...
def _update_lease_row(connection: sqlite3.Connection, *, lease_id: str, owner: str, expires_at: str, payload: dict[str, Any]) -> None:
    connection.execute(
        """
        update leases
...
def _lease_record(*, lease_id: str, subject: str, owner: str, expires_at: str, payload: dict[str, Any]) -> dict[str, Any]:
    return {"id": lease_id, "subject": subject, "expires_at": expires_at, **lease_contract_fields(payload), "payload": payload}
