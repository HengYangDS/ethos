...
def apply_local_state_maintenance(
    ...
    if not confirm_irreversible:
        message = "maintenance_irreversible_confirmation_required"
    ...
    if inventory["inventory_digest"] != expect_inventory_digest:
        message = "maintenance_inventory_digest_mismatch"
        raise ValueError(message)
...
def _validated_roots(root: Path, archive_root: Path) -> tuple[Path, Path]:
    if not archive_root.is_absolute():
        message = "maintenance_archive_root_must_be_absolute"
        raise ValueError(message)
    repo = root.resolve()
    archive = archive_root.resolve()
    if archive == repo or archive.is_relative_to(repo):
        message = "maintenance_archive_root_must_be_external"
        raise ValueError(message)
    return repo, archive
...
def _normalized_observed_at(value: datetime | str) -> datetime:
    try:
        ...
        message = "maintenance_observed_at_invalid"
        raise ValueError(message) from exc
    if observed.tzinfo is None:
        message = "maintenance_observed_at_timezone_required"
        raise ValueError(message)
...
def _git_lines(root: Path, *args: str) -> list[str]:
    ...
    if completed.returncode != 0:
        message = f"maintenance_git_observation_failed:{args[0]}"
...
def _database_inventory(db_path: Path) -> dict[str, Any]:
    if not db_path.exists():
        return {
            "path": db_path.as_posix(), "exists": False, "digest": "", "schema_versions": [],
    ...
    try:
        ...
        return {
            "path": db_path.as_posix(), "exists": True, "digest": "", "schema_versions": [],
            "target_schema_version": SCHEMA_VERSION,
            "cache_entries": {"exists": False, "row_count": 0}, "error": exc.__class__.__name__,
        }
    return {
        "path": db_path.as_posix(), "exists": True, "digest": digest, "schema_versions": versions,
        "target_schema_version": SCHEMA_VERSION,
...
def _lease_retention_reasons(
    root: Path, row: dict[str, Any], *, observed: datetime,
...
def _tree_entries(root: Path) -> list[dict[str, Any]]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.is_symlink():
            message = f"maintenance_archive_symlink_unsupported:{relative}"
            raise ValueError(message)
        if path.is_dir():
            ...
            message = f"maintenance_archive_entry_unsupported:{relative}"
            raise ValueError(message)
    return entries
...
def _verify_archive_extraction(
    archive: Path, manifest: dict[str, Any], *, repository_root: Path,
) -> dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="ethos-archive-verify-") as temp:
        ...
        if _tree_entries(extracted) != manifest["entries"]:
            message = "maintenance_archive_entry_verification_failed"
            raise RuntimeError(message)
...
def _verify_bundles(root: Path, *, cwd: Path) -> list[dict[str, Any]]:
    ...
    for bundle in sorted(root.rglob("*.bundle")):
        ...
        if completed.returncode != 0:
            message = f"maintenance_bundle_verify_failed:{relative}"
            raise RuntimeError(message)
...
def _delete_recovery_snapshot(root: Path, recovery: dict[str, Any]) -> bool:
    ...
    if not source.is_dir() or _tree_entries(source) != recovery["entries"]:
        message = "maintenance_recovery_snapshot_drift"
        raise ValueError(message)
...
def _verified_existing_receipt(archive_root: Path, digest: str, repository_root: Path) -> dict[str, Any] | None:
    ...
    try:
        ...
        message = "maintenance_existing_receipt_invalid"
        raise ValueError(message) from exc
