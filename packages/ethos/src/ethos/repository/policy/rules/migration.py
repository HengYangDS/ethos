...
def migrate_legacy_rules(root: Path, *, apply: bool = False, expect_source_digest: str | None = None, expect_head: str | None = None, read_head: Callable[[], str] | None = None) -> dict[str, object]:
    ...
    if migration_gaps:
        return _migration_error(source_text, source_digest, migration_gaps, source=source, legacy_detected=legacy_detected)
    try:
        target_text = _migrated_rules_text(source_text, source, profile_stack) if legacy_detected else source_text
        ...
        return _migration_error(source_text, source_digest, f"rules_migration_target_parse_error:{exc}", source=source, legacy_detected=legacy_detected)
    target_gaps = _target_rule_migration_gaps(target)
    if target_gaps:
        return _migration_error(source_text, source_digest, target_gaps, source=source, legacy_detected=legacy_detected)
    ...
    if apply and legacy_detected and not required_gaps:
        ...
        write_gap = _compare_and_swap_rules(path, expected_digest, target_text, expect_head=expect_head, read_head=read_head)
        if write_gap is not None:
            required_gaps.append(write_gap)
    applied = bool(apply and legacy_detected and not required_gaps)
    return {"ok": not required_gaps, "legacy_detected": legacy_detected, "applied": applied, "source_digest": source_digest, "target": target, "target_text": target_text, "required_gaps": required_gaps, "next_actions": (["ethos rules migrate --apply --authorize --expect-head <git-head>"] if legacy_detected and not apply else [])}
...
def _migration_error(source_text: str, source_digest: str, gaps: str | list[str], *, source: dict[str, Any] | None = None, legacy_detected: bool = False) -> dict[str, object]:
    return {"ok": False, "legacy_detected": legacy_detected, "applied": False, "source_digest": source_digest, "target": source or {}, "target_text": source_text, "required_gaps": [gaps] if isinstance(gaps, str) else gaps, "next_actions": ["repair .ethos/rules.toml before migration"]}
...
def _compare_and_swap_rules(path: Path, expected_digest: str, target_text: str, *, expect_head: str | None, read_head: Callable[[], str] | None) -> str | None:
    ...
    with lock_path.open("a+", encoding="utf-8") as lock:
        ...
        return _write_text_atomic(path, expected_digest, target_text, expect_head=expect_head, read_head=read_head)


def _write_text_atomic(path: Path, expected_digest: str, text: str, *, expect_head: str | None, read_head: Callable[[], str] | None) -> str | None:
...
def _head_guard_gap(expect_head: str | None, read_head: Callable[[], str] | None) -> str | None:
...
def _prepared_temporary(path: Path, text: str) -> Path:
    ...
    descriptor, raw_path = tempfile.mkstemp(dir=path.parent, prefix=f".{path.name}.migration.")
...
_SUPPORTED_RULE_KEYS = {"id", "version", "owner", "profile_layers", "authority_ref", "contract_ref", "subject", "path_globs", "severity", "required_gates", "evidence_requirements", "stop_condition", "non_waivable", "risk", "paths", "requires", "evidence"}
_RULE_KEY_PAIRS = (("paths", "path_globs"), ("requires", "required_gates"), ("evidence", "evidence_requirements"))
_STRING_RULE_KEYS = {"id", "owner", "authority_ref", "contract_ref", "subject", "severity", "stop_condition", "risk"}
_STRING_ARRAY_RULE_KEYS = {"profile_layers", "path_globs", "required_gates", "evidence_requirements", "paths", "requires", "evidence"}


def _legacy_state_from_source(source: dict[str, Any]) -> dict[str, object]:
    rules = source.get("rule")
    legacy_rule_items = isinstance(rules, list) and any(isinstance(item, dict) and is_legacy_rule_item(item) for item in rules)
    has_v2_rules = isinstance(source.get("profiles"), dict) or (isinstance(rules, list) and not legacy_rule_items)
    return {"legacy_detected": legacy_rule_items or (bool(_LEGACY_TOP_LEVEL_KEYS.intersection(source)) and not has_v2_rules), "has_v2_rules": has_v2_rules, "legacy_rule_items": legacy_rule_items}


def _legacy_rule_migration_gaps(root: Path, source: dict[str, Any]) -> list[str]:
...
def _target_rule_migration_gaps(target: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(raw_rules, list):
        return []
    return [f"rules_migration_target_legacy_keys:{item.get('id') or f'<index:{index}>'}" for index, item in enumerate(raw_rules) if isinstance(item, dict) and is_legacy_rule_item(cast("dict[str, Any]", item))]


def _legacy_rule_type_gaps(rule_id: str, item: dict[str, Any]) -> list[str]:
    gaps = [f"rules_migration_invalid:{rule_id}:{key}_must_be_string" for key in sorted(_STRING_RULE_KEYS.intersection(item)) if not isinstance(item[key], str)]
    gaps.extend(f"rules_migration_invalid:{rule_id}:{key}_must_be_string_array" for key in sorted(_STRING_ARRAY_RULE_KEYS.intersection(item)) if not isinstance(item[key], list) or any(not isinstance(value, str) for value in item[key]))
    if "version" in item and (not isinstance(item["version"], int) or isinstance(item["version"], bool)):
...
def _migrated_rules_text(source_text: str, source: dict[str, Any], active_profiles: list[str]) -> str:
...
def _rule_toml_lines(rule: dict[str, Any]) -> list[str]:
    lines = ["[[rule]]"]
    for key in ("id", "owner", "authority_ref", "contract_ref", "subject", "severity", "stop_condition"):
        value = cast("str", rule[key])
        lines.append(f'{key} = "{_toml_escape(value)}"')
    version = rule.get("version")
    if isinstance(version, int) and version != 1:
        lines.append(f"version = {version}")
    for key in ("profile_layers", "path_globs", "required_gates", "evidence_requirements"):
        value = rule.get(key)
