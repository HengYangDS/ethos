...
# ruff: noqa: E501 - compact lifecycle surface; Cyclopts resolves runtime annotations.
# fmt: off
...
def _closeout_result(payload: _CloseoutPayload) -> EthosResult:
    ...
    return EthosResult(command='land', ok=payload.ok, state='accepted_current' if payload.ok and payload.decision.state == 'current' else 'ready_to_closeout' if payload.ok and (not payload.mutation.apply) else 'deferred' if payload.control_replacement.get('verdict') == 'defer' else 'blocked' if payload.gaps else str(payload.update.get('state') or payload.mutation.command), required_gaps=payload.gaps, next_actions=mutation_next_actions, governance_context=context_for_root(payload.audit_root), data={'repository_audit': payload.audit, 'openspec_lifecycle': payload.lifecycle, 'accepted_update': payload.update, 'control_replacement': payload.control_replacement, 'closeout_bootstrap': land_core.closeout_bootstrap_package(repo=payload.repo, audit_root=payload.audit_root, required_gaps=payload.gaps), 'mutation': mutation_envelope(payload.mutation, action='accepted.advance', resource=f'refs/heads/{load_branch_role_policy(payload.repo).accepted_branch}', expected_state=_closeout_expected_state(payload), verdict='allow' if payload.ok else 'defer' if payload.control_replacement.get('verdict') == 'defer' else 'block', required_gaps=payload.gaps, why=('candidate_already_current',) if payload.ok and payload.decision.state == 'current' else (), next_actions=mutation_next_actions, state=payload.decision.state)})
...
def _publish_expected_state(*, repo: Path, branch: str, current_head: str, publication: Mapping[str, object], remote_observations: Mapping[str, object], branch_admission: Mapping[str, object]) -> dict[str, object]:  # noqa: PLR0913, RUF100 - exact publication state dimensions
    ...
    targets = [{'id': key, 'remote': str(_object_mapping(data.get('availability')).get('remote') or ''), 'availability_state': str(_object_mapping(data.get('availability')).get('state') or 'not_probed'), 'sync_state': str(_object_mapping(data.get('sync')).get('state') or 'not_checked'), 'observed_remote_ref': str(_object_mapping(data.get('sync')).get('remote_ref') or ''), 'observed_remote_head': str(_object_mapping(data.get('sync')).get('remote_head') or '')} for key, data in observations.items()]
    return {'root': repo.resolve().as_posix(), 'source_ref': f'refs/heads/{branch}', 'source_head': current_head, 'target_ref': f'refs/heads/{target_branch}', 'remote': str(availability.get('remote') or 'origin'), 'observed_remote_ref': str(sync.get('remote_ref') or ''), 'observed_remote_head': str(sync.get('remote_head') or ''), 'remote_availability_state': str(availability.get('state') or 'not_probed'), 'remote_sync_state': str(sync.get('state') or 'not_checked'), 'remote_targets': targets, 'branch_admission': dict(branch_admission)}
...
def land(*, apply: bool=False, authorize: bool=False, expect_head: str | None=None, closeout: bool=False, control_verifier_receipt: Annotated[Path | None, Parameter(name='--control-verifier-receipt')]=None, root: RootOption | None=None, json_output: JsonFlag=False) -> None:  # noqa: PLR0913, RUF100 - stable CLI contract preserves public options
    ...
    if closeout:
        ...
        if ok and apply:
            ...
            control_replacement = control_replacement_report(accepted_root=repo, candidate_root=candidate_root, accepted_head=current_head, candidate_head=str(fresh_candidate.get('head') or current_head), external_receipt=control_verifier_receipt)
            ...
            if fresh_control_gaps:
                ...
                ok = False
        if ok and apply:
            update = apply_candidate_to_accepted(root=repo, authorized=authorize, expect_head=expect_head)
    ...
    request = MutationRequest(command='land', apply=apply, authorized=authorize, expect_head=expect_head)
    decision = evaluate_mutation(request, root=repo, current_head=current_head, status=None if apply else status_payload)
    ...
    update: dict[str, object] = {}
    if ok and apply:
        update = apply_land_to_candidate(root=repo, authorized=authorize, expect_head=expect_head, admitted_decision=decision)
        gaps = gaps + tuple(string_sequence(update.get('required_gaps')))
        ok = bool(update['ok'])
    elif ok:
    ...
    state = 'ready_to_land' if ok and (not apply) else 'blocked' if gaps else str(update.get('state') or decision.state)
    mutation_next_actions = land_core.land_next_actions(ok=ok, gaps=gaps, current_head=current_head)
    result = EthosResult(command='land', ok=ok, state=state, required_gaps=gaps, next_actions=mutation_next_actions, governance_context=governance, data={'repository_audit': audit, 'openspec_lifecycle': lifecycle, 'candidate_update': update, 'closeout_support': closeout_support, 'proof_readiness': proof_readiness, 'mutation': mutation_envelope(request, action='candidate.integrate', resource=f'refs/heads/{load_branch_role_policy(repo).candidate_branch}', expected_state=_land_expected_state(repo=repo, current_head=current_head, status_payload=status_payload, closeout_support=closeout_support), verdict='allow' if ok else 'block', required_gaps=gaps, next_actions=mutation_next_actions, state=state)})
    emit(result, json_output=json_output, enforce=apply)
...
def publish(options: Annotated[_PublishOptions, Parameter(name='*')]=_DEFAULT_PUBLISH_OPTIONS, *, root: RootOption | None=None, json_output: JsonFlag=False) -> None:
    ...
    publish_summary = {'mode': 'local_readiness', 'local_readiness': ok, 'remote_push': remote_push, 'remote_publication_state': remote_state, 'remote_availability_state': remote_availability_state, 'remote_sync_state': str(remote_sync.get('state') or 'not_checked'), 'remote_reconciliation_state': str(remote_matrix.get('state') or 'pending'), 'gitlab_remote_state': str(remote_availability.get('state') or 'not_probed'), 'github_remote_state': str(_object_mapping(remote_observations['github'].get('availability')).get('state') or 'not_probed'), 'remote_mutation_allowed': bool(branch_admission.get('remote_mutation_allowed')), 'remote_ahead': _int_value(remote_sync.get('ahead')), 'remote_behind': _int_value(remote_sync.get('behind')), 'hosted_ci_status_claimed': False, 'independent_verification': str(independent_verification.get('evidence_class') or 'local_readiness'), 'submit_branch': str(publication.get('submit_branch') or ''), 'next_publication_action': next(iter(string_sequence(publication.get('next_actions'))), '')}
