...
# ruff: noqa: E501 - compact proof surface preserves the payload contract.
# fmt: off
...
KNOWN_PROOF_SCOPES = frozenset({'repository', 'proof-kernel', 'code', 'docs', 'openspec', 'quality'})

def missing_gate_dependency_next_actions(*, selected_gate_ids: tuple[str, ...], validation_gaps: tuple[str, ...], current_head: str, root: Path | None=None) -> tuple[str, ...]:
    """Return a concrete proof rerun command when selected gates omit dependencies."""
    if not selected_gate_ids:
        return ()
    missing_dependencies = tuple(gap.removeprefix('missing_dependency:').split('->', maxsplit=1)[1] for gap in validation_gaps if gap.startswith('missing_dependency:') and '->' in gap)
...
def proof_scope_binding(scope: str) -> dict[str, object]:
    ...
    return {'scope': normalized, 'accepted': known, 'known': known, 'known_scopes': sorted(KNOWN_PROOF_SCOPES), 'semantics': 'proof-boundary metadata; gate selection remains controlled by --gate/--full/profile', 'required_gaps': [] if known else [f'unknown_proof_scope:{normalized}']}
...
def host_probe_boundary(*, host: bool, probe: bool) -> dict[str, object]:
    ...
    return {'requested': host or probe, 'host': host, 'probe': probe, 'evidence_class': 'optional_host_readiness', 'satisfies_repository_proof': False, 'truth_boundary': 'host-local projection', 'state': 'not_requested' if not (host or probe) else 'boundary_recorded'}

def prove(*, objective: str='ethos proof', scope: str='repository', execute: bool=False, gate: tuple[str, ...]=(), full: bool=False, expect_head: str | None=None, host: bool=False, probe: bool=False, root: RootOption | None=None, json_output: JsonFlag=False) -> None:  # noqa: PLR0913, RUF100 - stable CLI contract preserves public options
    ...
    runs_ok = verdicts_ok and trust_bearing_ok if execute else all(run.state == 'planned' for run in proof_runs)
    failed_gate_gaps: tuple[str, ...] = tuple(f'gate_failed:{run.action_id}' for run in proof_runs if run.verdict != 'passed') if execute else ()
    proof_gaps: tuple[str, ...] = ('full_proof_requires_execute',) if full and (not execute) else ()
    trust_gaps: tuple[str, ...] = ('trust_bearing_proof_missing',) if execute and verdicts_ok and (not trust_bearing_ok) else ()
    head_gaps: tuple[str, ...] = ('expected_head_mismatch',) if expect_head is not None and expect_head != current_head else ()
