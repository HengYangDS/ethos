"""Native fact observation and re-observation for ownerless closeout."""
...
import ethos.adapters.mutation.resolution.closeout.ownerless.admission.fence as admission_fence
import ethos.adapters.mutation.resolution.closeout.ownerless.receipt.reservation as receipt
import ethos.adapters.mutation.resolution.closeout.ownerless.workspace as workspace
import ethos.adapters.mutation.resolution.observation as git
import ethos.adapters.mutation.resolution.records.current.validators as validation
...
_ADMISSION_UNVERIFIABLE = "lane_resolution_ownerless_admission_unverifiable"
...
def admit_ownerless_closeout(
    *,
    ...
    """Admit one exact clean ownerless target from native repository facts."""
    try:
        return _admit_ownerless_closeout_facts(
            root=root,
        ...
        raise admission_fence.OwnerlessCloseoutAdmissionError(
            _ADMISSION_UNVERIFIABLE, error.__class__.__name__
        ) from error


def reobserve_ownerless_closeout(
    *,
    admission: admission_fence.OwnerlessCloseoutAdmission,
    fence: dict[str, object],
    receipt_reservation: receipt.OwnerlessReceiptReservationContext | None = None,
) -> admission_fence.OwnerlessCloseoutAdmission:
    """Require the exact fence before and after complete native re-observation."""
    try:
        return _reobserve_ownerless_closeout_facts(
            admission=admission,
            fence=fence,
            receipt_reservation=receipt_reservation,
        )
...
def _admit_ownerless_closeout_facts(
    *,
    root: Path,
    decision_path: Path,
    decision: dict[str, Any],
    ...
    binding = admission_fence.ownerless_fence_binding(
        model=model, observation=facts.observation, authority=authority
    )
    binding_digest = admission_fence.ownerless_fence_digest(binding)
    ...
    if reservation_gap:
        ...
        raise admission_fence.OwnerlessCloseoutAdmissionError(reservation_gap, detail)
    ...
    if fence_observation is not None or observed_fence[0] == "present":
        ...
        retry_fence = admission_fence.ownerless_closeout_fence(
            model, facts.observation, fence_authority, acquisition_id
        )
    ...
    return admission_fence.OwnerlessCloseoutAdmission(
        root=control,
...
def _reobserve_ownerless_closeout_facts(
    *,
    admission: admission_fence.OwnerlessCloseoutAdmission,
    fence: dict[str, object],
    ...
    try:
        with receipt.ownerless_receipt_reservation_guard(receipt_reservation):
            ...
            expected = admission_fence.ownerless_admission_fence(admission, acquisition_id)
            _exact_fence(before, expected, "before", supplied=fence)
            try:
                fresh = _admit_ownerless_closeout_facts(
                    root=admission.root,
                ...
                for field in fields(admission_fence.OwnerlessCloseoutAdmission):
...
def _authority_context(root: Path, executor_ref: str) -> tuple[roles.BranchRolePolicy, str]:
    ...
    try:
        snapshot = workspace.read_optional_root_bound_regular_file(
...
def _error(suffix: str, detail: str = "") -> admission_fence.OwnerlessCloseoutAdmissionError:
    return admission_fence.OwnerlessCloseoutAdmissionError(
        f"lane_resolution_ownerless_{suffix}", detail
    )
...
def _fail(suffix: str, detail: str = "") -> NoReturn:
    ...
    raise admission_fence.OwnerlessCloseoutAdmissionError(gap, detail)
