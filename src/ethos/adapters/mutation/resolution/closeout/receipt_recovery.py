...
def ownerless_receipt_recovery_context(
    *,
    ...
    try:
        ...
        return {}, classify_closeout_failure(error, "lane_resolution_receipt_invalid")
    if current is None:
        return {}, ""
...
def ownerless_reservation_recovery_context(
    *,
    ...
    try:
        ...
        return {}, classify_closeout_failure(error, "lane_resolution_ownerless_reservation_invalid")
    exact = (
...
def recover_existing_ownerless_receipt(
    *,
    ...
    expected_binding = {
        field: (context.reservation or {})[field] for field in _OWNERLESS_RECEIPT_FIELDS
    }
    if not exact_ownerless_resolution_receipt(
        receipt=receipt,
    ...
    try:
        ...
        _block(
            report,
            classify_closeout_failure(error, "lane_resolution_ownerless_recovery_not_finalizable"),
            state="partial_transition",
        )
        return True
...
def release_ownerless_closeout_resources(
    *,
    ...
    expected = {
        "schema_version": 2,
        "decision_id": str(context.decision.get("decision_id") or ""),
        "lane_ref": context.observation.lane_ref,
        "head": context.observation.head,
        **{
            field: binding[field]
    ...
    database = state_database(context.control_root)
    try:
        try:
            release_closeout_fence(
                database,
                subject=context.observation.lane_ref,
                decision_id=str(context.decision.get("decision_id") or ""),
                target_binding_digest=str(binding["target_binding_digest"]),
            )
        except ValueError as error:
            fence_state, _fence = probe_closeout_fence(
                database, subject=context.observation.lane_ref
            )
