...
from ethos.adapters.mutation.resolution.records.clear.quarantine import clear_quarantine_name
...
def _request_gaps(request: LaneResolutionClearRequest, actual_manifest_sha256: str) -> list[str]:
    return [
        *(
            ...
            if not is_lane_decision_id(request.decision_id)
            else []
        ),
...
def _apply_clear_plan(
    ...
    clear_path = _clear_receipt_path(root, request.decision_id)
    if not record_path_is_safe(observation.record_root, clear_path):
        return _failed(report, "blocked", "lane_resolution_clear_receipt_path_unsafe")
    if observation.terminal:
        report.update(state="cleared", clear_receipt_path=display_record_path(root, clear_path))
        return report
    ...
    report["clear_receipt_path"] = display_record_path(root, clear_path)
...
def _prepare_quarantine(
    ...
    quarantine_path = _clear_quarantine_path(
        root,
        decision_id,
        package_identity,
        artifact_root=observation.record_root,
    )
    if not record_path_is_safe(observation.record_root, quarantine_path):
...
def _clear_chronicle(root: Path, chronicle_ref: str) -> tuple[str, str, list[str]]:
    ...
    return relative, hashlib.sha256(candidate.read_bytes()).hexdigest(), []
