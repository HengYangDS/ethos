...
from ethos.adapters.mutation.resolution.records.clear.quarantine import clear_quarantine_identity
...
from ethos.adapters.mutation.resolution.records.current.validators import canonical_record_name
from ethos.adapters.mutation.resolution.records.current.validators import cross_record_invalid_paths
from ethos.adapters.mutation.resolution.records.current.validators import validate_clear_receipt
from ethos.adapters.mutation.resolution.records.current.validators import validate_decision
from ethos.adapters.mutation.resolution.records.current.validators import validate_receipt
from ethos.adapters.mutation.resolution.records.current.validators import validate_reservation
    ...
    for source in sources:
        ...
        projected = {
            **payload,
            ...
            "record_path": display_record_path(root, source.path),
...
def _manifests_with_conflicts(
    ...
    for source in sources:
        ...
        records[decision_id] = {
            "physical_path": source.path,
            "package_path": display_record_path(root, source.path.parent),
            "record_path": display_record_path(root, source.path),
            "copy_count": 1,
...
def _receipt_reservations(
    ...
    for source in sources:
        ...
        if (
            not is_lane_decision_id(decision_id)
        ...
        records.setdefault(
            decision_id,
            {
                ...
                "reservation_path": display_record_path(root, source.path),
                "phase": "unknown",
