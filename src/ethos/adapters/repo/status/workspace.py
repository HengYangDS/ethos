...
def _status_payload(payload: _StatusPayload) -> dict[str, object]:
    result: dict[str, object] = {"root": str(payload.root), "branch": payload.branch}
    if payload.head is not None:
        result["head"] = payload.head
    result.update(
        {
            "dirty": bool(payload.paths),
            "changed_paths": list(payload.paths),
            ...
            "required_gaps": payload.workspace_gaps,
        }
    )
    return result
...
def worktree_records(
    ...
    blocks = (
        block
        for block in git_stdout_checked(root, "worktree", "list", "--porcelain").split("\n\n")
        if block.strip()
    )
    return [
        _normalize_worktree(
    ...
    *,
    ...
    for worktree in worktrees:
        ...
        if not include_path_scope:
            foreign.append(foreign_work_lane_deferred(worktree, lease=lease))
            continue
        dirty_paths = (
            ()
...
def _candidate_status(
    ...
    return {
        ...
        "exists": bool(head),
        "head": head,
        "worktree_exists": binding in {"current", "linked"},
        "worktree_path": path,
