"""Compile repository intent into transport-independent planning results."""
...
@application_result("plan")
def plan_repository(root: Path, *, changed: bool = False, change: str | None = None) -> EthosResult:
    ...
    generation = resolve_current_resolution(
        repo, status=status_payload, authority=authority, change=change, changed=changed
    )
    ...
    return EthosResult(
        ...
        or (
            "ethos prove --json"
            if ok
            else "repair .ethos/rules.toml and rerun ethos plan --json"
