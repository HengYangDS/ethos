...
_IMMUTABLE_RECORD_ROOTS = ("evidence/", "openspec/changes/archive/")
_IMMUTABLE_RECORD_ROOTS_CONTRACT_VERSION = 2
...
class Policy(_Contract):
    ...
    def validate_ownership(self) -> Policy:
        ...
        if self.contract_version == 1 and self.immutable_record_roots:
            ...
            raise ValueError(msg)
        if (
            self.contract_version == _IMMUTABLE_RECORD_ROOTS_CONTRACT_VERSION
            and self.immutable_record_roots != _IMMUTABLE_RECORD_ROOTS
        ):
...
def _pairs(value: object) -> tuple[tuple[str, str], ...]:
    ...
    for raw in _sequence(value):
        values = _strings(raw)
        if len(values) != _PAIR_MEMBER_COUNT:
            raise TypeError
...
def _relaxed(current: Policy, accepted: Policy) -> bool:
    if (accepted.contract_version, current.contract_version) not in {
        (1, 1),
        (1, 2),
        (2, 2),
    }:
