...
import ethos.repository.policy.coupling.command_references as command_references
import ethos.repository.policy.coupling.python_references as python_references
...
def product_references_from_files(
    ...
    *,
    ...
    if not declared_commands and root is not None and (root / "system/coupling.toml").is_file():
        declaration = load_coupling_declaration(root / "system/coupling.toml")
        declared_commands = tuple(declared_product_references(declaration)["command"])
        ...
        for tree in python_references.python_trees(text):
            ...
            if not path.startswith("tests/"):
                observed["executable"].update(
                    python_references.python_executables(tree, npm_scripts)
                )
...
def _markdown_references(
    ...
    for match in _MARKDOWN_FENCE.finditer(text):
        ...
        if language in {"bash", "console", "sh", "shell", "zsh"}:
            ...
            observed["command"].update(command_references.shell_commands(shell, known_commands))
    ...
    for inline in _MARKDOWN_INLINE_CODE.findall(text):
        try:
            ...
            continue
        if command := command_references.command_identity(tokens, known_commands):
