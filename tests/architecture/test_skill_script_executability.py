...
def test_repository_governance_script_supports_documented_root_forms() -> None:
    """The governance skill advertises a script; its parser should accept both forms."""
    script = ROOT / ".agents/skills/ethos-repository-governance/scripts/govern_check.py"
    text = script.read_text(encoding="utf-8")
    assert "root_arg" in text
    assert "--root" in text
    assert "govern_check.py [ROOT]" in text
