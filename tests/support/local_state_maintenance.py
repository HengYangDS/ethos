...
def insert_lease(repo: Path, *, lease_id: str, subject: str, expires_at: str, payload: dict[str, object] | str) -> None:
...
def current_lease_payload(*, path: str='', expected_head: str='') -> dict[str, object]:
    return {'lease_id': 'lease:fixture', 'lane_incarnation_id': 'lane-incarnation:fixture', 'lane_ref': 'work/fixture', 'holder_ref': 'agent:test:case:owner', 'epoch': 1, 'issued_at': '2026-07-01T00:00:00+00:00', 'renewed_at': '2026-07-01T00:00:00+00:00', 'expected_head': expected_head, 'claim_id': '', 'path_scope': [], 'coordination_scope': 'git_common_directory', 'mints_authority': False, 'filesystem_fence': False, 'distributed_lock': False, 'path': path}
...
def unreachable_commit(repo: Path) -> str:
    ...
    return git(repo, '-c', 'user.name=Test User', '-c', 'user.email=test@example.com', 'commit-tree', tree, '-m', 'unreachable maintenance proof')
