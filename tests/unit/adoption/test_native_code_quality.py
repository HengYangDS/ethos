...
@pytest.mark.parametrize("mode", ["native", "self-report"])
def test_public_proof_qualifies_one_native_javascript_test_run(tmp_path: Path, mode: str) -> None:
    """The selected native gate must emit real JUnit/V8, not provider-shaped stdout."""
    if shutil.which("node") is None:
        pytest.skip("node is unavailable on this runner")
    repo = init_git_repo(tmp_path / "adopter with spaces")
    _write_javascript_fixture(repo, "none")
    command = (
        ["node", "--test", "answer.test.js"]
        if mode == "native"
        else ["node", "-e", "console.log(JSON.stringify({providers: []}))"]
    )
    profile = repo / ".ethos/profile.toml"
    profile.parent.mkdir()
    profile.write_text(
        'profile_id = "native-output-adopter"\n\n'
        '[openspec]\nmaterial_paths = ["**"]\n\n'
        '[proof]\ncode_correctness_gates = ["behavior", "static"]\n\n'
        '[proof.code_correctness_map]\nbehavior = "behavior"\n'
        'static-analysis = "static"\n\n'
    ...
    head = commit_fixture(repo, "declare one native code gate")
