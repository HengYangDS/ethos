...
def test_land_closeout_apply_fast_forwards_accepted_root_from_candidate(tmp_path: Path) -> None:
    ...
    payload = run_ethos('land', '--closeout', '--apply', '--authorize', '--expect-head', accepted_head, '--json', cwd=repo)
    assert payload['ok'] is True
    assert payload['state'] == 'accepted_validated'
    assert payload['required_gaps'] == []
    assert payload['next_actions'] == ['ethos lane retire landed --branch <work-branch> --expect-head <work-lane-head>']
...
def test_land_closeout_defers_control_replacement_without_incumbent_receipt(tmp_path: Path) -> None:
    ...
    git(candidate, 'add', '.')
    git(candidate, '-c', 'user.name=Test User', '-c', 'user.email=test@example.com', 'commit', '-m', 'replace admission control')
    candidate_head = git(candidate, 'rev-parse', 'HEAD')
    seed_executed_proof(candidate, candidate_head)
    payload = run_ethos_blocked('land', '--closeout', '--apply', '--authorize', '--expect-head', git(repo, 'rev-parse', 'HEAD'), '--json', cwd=repo)
...
def test_land_closeout_audits_candidate_content_before_fast_forward(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_audit(root: Path, *, openspec_mode: str='shape', current_head: str='') -> dict[str, object]:
        ...
        return {'ok': False, 'required_gaps': ['accepted_root_precloseout_audit'], 'root': root.as_posix()}
...
def test_land_closeout_dry_run_reports_accepted_root_required(tmp_path: Path) -> None:
    ...
    git(repo, 'worktree', 'add', '-b', 'candidate/dev', candidate.as_posix(), 'dev')
    accepted_head = git(repo, 'rev-parse', 'HEAD')
    payload = run_ethos('land', '--closeout', '--root', candidate.as_posix(), '--expect-head', accepted_head, '--json', cwd=repo)
    assert payload['ok'] is False
    assert payload['state'] == 'blocked'
...
def test_land_closeout_dry_run_reports_current_when_candidate_matches_accepted(tmp_path: Path) -> None:
    repo = init_git_repo(tmp_path / 'repo')
    adopt_and_commit(repo)
    candidate = tmp_path / 'repo-candidate-dev'
    git(repo, 'worktree', 'add', '-b', 'candidate/dev', candidate.as_posix(), 'dev')
    ...
    assert mutation['request'] == {'command': 'closeout', 'apply': False, 'confirmation_present': False, 'expect_head': None}
...
def test_land_closeout_apply_is_noop_when_candidate_matches_accepted_without_proof(tmp_path: Path) -> None:
    repo = init_git_repo(tmp_path / 'repo')
    adopt_and_commit(repo)
    candidate = tmp_path / 'repo-candidate-dev'
    git(repo, 'worktree', 'add', '-b', 'candidate/dev', candidate.as_posix(), 'dev')
...
def test_land_closeout_exposes_bootstrap_package_for_current_runner(tmp_path: Path) -> None:
    ...
    assert runner_binding['runner_matches_accepted_root'] == bootstrap['runner_matches_accepted_root']
...
def test_land_closeout_bootstrap_proof_target_stays_candidate_when_blocked(tmp_path: Path) -> None:
    ...
    accepted_head = git(repo, 'rev-parse', 'HEAD')
    candidate_head = git(candidate, 'rev-parse', 'HEAD')
    payload = run_ethos_blocked('land', '--closeout', '--apply', '--authorize', '--expect-head', accepted_head, '--json', cwd=repo)
    bootstrap = payload['data']['closeout_bootstrap']
    assert payload['ok'] is False
    assert payload['required_gaps'] == ['proof_not_proven']
    assert bootstrap['audit_root'] == repo.resolve().as_posix()
    assert bootstrap['proof_target'] == {'kind': 'closeout_proof_target', 'role': 'candidate', 'root': candidate.resolve().as_posix(), 'head': candidate_head, 'reason': 'accepted-root closeout promotes the candidate head'}

def test_land_closeout_blocks_candidate_with_completed_active_openspec_change(tmp_path: Path, monkeypatch) -> None:
    ...
    def fake_openspec_lifecycle(root: Path) -> dict[str, object]:
        ...
        return {'ok': True, 'state': 'clean', 'root': root.as_posix(), 'completed_changes': [], 'required_gaps': []}
    monkeypatch.setattr('ethos.domain.status.audit_for_root', fake_audit)
