...
def test_land_dry_run_reports_dirty_work_lane_gap(tmp_path: Path) -> None:
    ...
    run_ethos('lane', 'start', 'feature', '--root', repo.as_posix(), '--path', worktree.as_posix(), '--holder-ref', 'agent:test:case:agent-test', '--apply', '--json', cwd=repo)
...
def test_land_blocks_completed_active_openspec_change_before_candidate_landing(tmp_path: Path, monkeypatch) -> None:
    import ethos.surface.cli.root.lifecycle as lifecycle_cli
    repo, _candidate = init_repo_with_candidate(tmp_path)
    worktree = tmp_path / 'repo-work-feature'
    run_ethos('lane', 'start', 'feature', '--root', repo.as_posix(), '--path', worktree.as_posix(), '--holder-ref', 'agent:test:case:agent-test', '--apply', '--json', cwd=repo)

    def fake_audit(root: Path, *, openspec_mode: str='shape', current_head: str='') -> dict[str, object]:
    ...
    def fake_openspec_lifecycle(root: Path) -> dict[str, object]:
        return {'ok': False, 'state': 'blocked', 'root': root.as_posix(), 'completed_changes': ['sample-change'], 'required_gaps': ['openspec_completed_change_unarchived:sample-change']}
    monkeypatch.setattr('ethos.domain.status.audit_for_root', fake_audit)
    monkeypatch.setattr(lifecycle_cli, 'completed_active_changes_report', fake_openspec_lifecycle, raising=False)
    payload = run_ethos('land', '--root', worktree.as_posix(), '--json', cwd=worktree)
    assert payload['ok'] is False
    assert payload['state'] == 'blocked'
...
def test_land_dry_run_reports_stale_candidate_base_with_refresh_action(tmp_path: Path) -> None:
    ...
    assert payload['next_actions'] == [f'ethos lane refresh-base --apply --authorize --expect-head {work_head} --json']
    assert payload['data']['candidate_update'] == {'ok': False, 'state': 'blocked', 'branch': 'candidate/dev', 'head': work_head, 'candidate_head': candidate_head, 'path': candidate.as_posix(), 'required_gaps': ['candidate_base_stale'], 'remediation': [{'gap': 'candidate_base_stale', 'kind': 'stale_base', 'next_actions': ['ethos lane refresh-base --apply --authorize --expect-head <head> --json', 'rerun proof after the lane is replayed onto candidate/dev']}]}
...
def test_land_dry_run_requires_executed_proof_before_ready_state(tmp_path: Path) -> None:
    ...
    assert payload['data']['proof_readiness'] == {'kind': 'executed_proof_readiness', 'head': work_head, 'state': 'missing', 'blocking': True, 'required_gaps': ['proof_not_proven'], 'next_action': f'ethos prove --execute --expect-head {work_head} --json', 'local_readiness': False, 'evidence_class': 'local_readiness', 'independent_verification': {'ok': True, 'state': 'disabled', 'root': worktree.resolve().as_posix(), 'mode': 'disabled', 'receipt': {}, 'evidence_class': 'local_readiness', 'mints_authority': False, 'required_gaps': []}}
...
def test_land_dry_run_reports_ready_after_executed_proof(tmp_path: Path) -> None:
    ...
    assert payload['data']['proof_readiness'] == {'kind': 'executed_proof_readiness', 'head': work_head, 'state': 'proven', 'blocking': False, 'required_gaps': [], 'next_action': '', 'local_readiness': True, 'evidence_class': 'local_readiness', 'independent_verification': {'ok': True, 'state': 'disabled', 'root': worktree.resolve().as_posix(), 'mode': 'disabled', 'receipt': {}, 'evidence_class': 'local_readiness', 'mints_authority': False, 'required_gaps': []}}
    mutation = payload['data']['mutation']
    assert mutation['request'] == {'command': 'land', 'apply': False, 'confirmation_present': False, 'expect_head': None}
...
def test_lane_refresh_base_apply_rebases_stale_work_lane(tmp_path: Path) -> None:
    ...
    candidate_head = git(candidate, 'rev-parse', 'HEAD')
    payload = run_ethos('lane', 'refresh-base', '--apply', '--authorize', '--expect-head', previous_head, '--json', cwd=worktree)
...
def test_land_apply_requires_authorization_and_expected_head(tmp_path: Path) -> None:
    ...
    assert 'decision' not in {key: value for key, value in mutation.items() if isinstance(value, str)}
...
def test_land_apply_rejects_accepted_root_even_when_authorized(tmp_path: Path) -> None:
    ...
    payload = run_ethos_blocked('land', '--apply', '--authorize', '--expect-head', head, '--json', cwd=repo)
    assert payload['ok'] is False
    assert payload['state'] == 'blocked'
...
def test_publish_dry_run_remains_available_on_accepted_root_after_land_boundary(tmp_path: Path) -> None:
    ...
    assert payload['required_gaps'] == []
    mutation = payload['data']['mutation']
    assert mutation['request'] == {'command': 'publish', 'apply': False, 'confirmation_present': False, 'expect_head': None}
    ...
    assert expected_state['remote_availability_state'] in {'unconfigured', 'unavailable', 'not_probed'}
...
def test_product_publish_blocks_current_hard_quality_gap(tmp_path: Path, monkeypatch) -> None:
    ...
    floor = {'ok': False, 'state': 'blocked', 'gate_ids': ['generated-artifacts'], 'required_gaps': [gap], 'gates': {}}
...
def test_publish_dry_run_blocks_release_root_active_openspec_residue(tmp_path: Path) -> None:
    ...
    git(repo, 'add', '.')
    git(repo, '-c', 'user.name=Test User', '-c', 'user.email=test@example.com', 'commit', '-m', 'leak active openspec carrier on release root')
...
def test_configured_branch_roles_drive_local_lifecycle_commands(monkeypatch, tmp_path: Path) -> None:
    ...
    candidate_payload = run_ethos('lane', 'candidate', '--root', repo.as_posix(), '--path', candidate_path.as_posix(), '--expect-head', accepted_head, '--apply', '--json', cwd=repo)
    ...
    start_payload = run_ethos('lane', 'start', 'configured', '--root', repo.as_posix(), '--path', worktree.as_posix(), '--holder-ref', 'agent:test:case:agent-test', '--apply', '--json', cwd=repo)
    ...
    closeout_payload = run_ethos('land', '--closeout', '--apply', '--authorize', '--expect-head', accepted_head, '--json', cwd=repo)
...
def test_publish_apply_rejects_accepted_root_even_when_authorized(tmp_path: Path) -> None:
    repo = init_git_repo(tmp_path / 'repo')
    head = git(repo, 'rev-parse', 'HEAD')
    payload = run_ethos_blocked('publish', '--apply', '--authorize', '--expect-head', head, '--json', cwd=repo)
    assert payload['ok'] is False
    assert payload['state'] == 'blocked'
    assert 'protected_root_mutation' in payload['required_gaps']
...
def test_publish_reports_current_local_ci_fallback_evidence_when_manifest_matches_head(tmp_path: Path) -> None:
    ...
    manifest.write_text(json.dumps({'schema_version': 1, 'kind': 'ethos_local_ci_fallback_evidence', 'ok': True, 'head': head, 'command': 'tools/ci/scripts/run-local-ci.sh'}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    payload = run_ethos('publish', '--json', cwd=repo)
...
def test_publish_reports_stale_local_ci_fallback_evidence_when_manifest_head_differs(tmp_path: Path) -> None:
    ...
    manifest = repo / 'build' / 'evidence' / 'local-ci' / 'fallback.json'
    manifest.parent.mkdir(parents=True)
    manifest.write_text(json.dumps({'schema_version': 1, 'kind': 'ethos_local_ci_fallback_evidence', 'ok': True, 'head': 'stale-head', 'command': 'tools/ci/scripts/run-local-ci.sh'}, indent=2, sort_keys=True) + '\n', encoding='utf-8')
    payload = run_ethos('publish', '--json', cwd=repo)
