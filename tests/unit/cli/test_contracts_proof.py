...
def test_prove_missing_gate_dependency_reports_concrete_rerun(tmp_path: Path) -> None:
    ...
    payload = run_ethos_blocked('prove', '--root', repo.as_posix(), '--gate', 'openspec', '--json')
    assert payload['ok'] is False
    assert 'missing_dependency:openspec->schemas' in payload['required_gaps']
    assert payload['next_actions'] == [f'ethos prove --execute --gate schemas --gate openspec --expect-head {head} --json']
...
def test_adopter_id_only_gate_declaration_fails_closed_without_traceback(tmp_path: Path) -> None:
    ...
    (repo / '.ethos/profile.toml').write_text('profile_id = "acme"\n[proof]\ncode_correctness_gates = ["acme-tests"]\n', encoding='utf-8')
...
def test_executed_proof_blocks_ethos_json_gate_failures(tmp_path: Path) -> None:
    docs = tmp_path / 'docs'
    docs.mkdir()
    (docs / 'guide.md').write_text('---\nsubject: sample:guide\nrole: how-to\nstate: active\nrelations: {}\n---\n\n# Guide\n\nBody without required visible sections.', encoding='utf-8')
    payload = run_ethos_blocked('prove', '--execute', '--gate', 'docs-registry', '--root', tmp_path.as_posix(), '--json')
    assert payload['ok'] is False
    assert payload['state'] == 'gapped'
    ...
    assert run['diagnostics'][0]['required_gaps'] == ['missing_visible_section:docs/guide.md:status', 'missing_visible_section:docs/guide.md:purpose', 'missing_visible_section:docs/guide.md:see also']
...
def test_prove_surfaces_active_archive_preflight_gap(monkeypatch) -> None:
    lifecycle = {'ok': False, 'required_gaps': ['openspec_archive_preflight_failed:sample-change:archive_spec_update_failed']}
    monkeypatch.setattr(proof_cli, 'openspec_governance_report', lambda _root, **_kwargs: lifecycle, raising=False)
...
def test_adopter_proof_surfaces_openspec_lifecycle_gap(monkeypatch, tmp_path: Path) -> None:
    ...
    lifecycle_payload = {'ok': False, 'required_gaps': ['openspec_claim_binding_missing:material-change']}
    calls: list[tuple[Path, bool, tuple[str, ...]]] = []

    def report(root: Path, *, lifecycle: bool=False, changed_paths: tuple[str, ...]=()) -> dict[str, object]:
    ...
    assert payload['data']['openspec_lifecycle']['required_gaps'] == lifecycle_payload['required_gaps']
...
def test_prove_accepts_host_probe_compatibility_flags_without_claiming_host_truth() -> None:
    payload = run_ethos('prove', '--scope', 'proof-kernel', '--host', '--probe', '--json')
    assert payload['ok'] is True
    assert payload['state'] == 'ready'
    assert payload['data']['scope'] == 'proof-kernel'
...
def test_prove_accepts_matching_expected_head() -> None:
    ...
    assert payload['data']['expected_head'] == {'expected': head, 'current': head, 'ok': True}
...
def test_init_apply_rejects_untracked_expected_head(tmp_path: Path) -> None:
    ...
    target.mkdir()
    payload = run_ethos_blocked('init', '--root', target.as_posix(), '--apply', '--authorize', '--expect-head', 'untracked', '--json')
    assert payload['ok'] is False
    assert payload['state'] == 'blocked'
    assert 'git_repository_missing' in payload['required_gaps']
    assert not (target / '.ethos' / 'project.toml').exists()
    assert payload['data']['mutation'] == {'apply': True, 'authorized': True, 'expect_head': 'untracked', 'current_head': 'untracked'}

def test_adopt_apply_rejects_untracked_expected_head(tmp_path: Path) -> None:
    target = tmp_path / 'sample'
    target.mkdir()
    payload = run_ethos_blocked('adopt', '--root', target.as_posix(), '--apply', '--authorize', '--expect-head', 'untracked', '--json')
    assert payload['ok'] is False
    assert payload['state'] == 'blocked'
    assert 'git_repository_missing' in payload['required_gaps']
    assert not (target / '.ethos' / 'project.toml').exists()
...
def test_init_apply_flag_applies_scaffold_in_git_repo(tmp_path: Path) -> None:
    ...
    payload = run_ethos('init', '--root', target.as_posix(), '--apply', '--authorize', '--expect-head', head, '--json')
    assert payload['ok'] is True
    ...
    examples = run_ethos('quality', 'command-examples', '--root', target.as_posix(), '--json')
    assert status['ok'] is True
    assert [gap for diagnostic in status['diagnostics'] if diagnostic['kind'] == 'schema_validation' for gap in diagnostic['required_gaps']] == []
...
def test_adopt_apply_accepts_authorized_matching_head(tmp_path: Path) -> None:
    repo = init_git_repo(tmp_path / 'repo')
    head = git(repo, 'rev-parse', 'HEAD')
    payload = run_ethos('adopt', '--root', str(repo), '--apply', '--authorize', '--expect-head', head, '--json', cwd=repo)
    assert payload['ok'] is True
    assert payload['state'] == 'applied'
    assert payload['data']['mutation'] == {'apply': True, 'authorized': True, 'expect_head': head, 'current_head': head}
    assert (repo / '.ethos/project.toml').exists()

def test_adopt_overlay_apply_preserves_existing_adopter_entrypoint(tmp_path: Path) -> None:
    repo = init_git_repo(tmp_path / 'repo')
    head = git(repo, 'rev-parse', 'HEAD')
...
def test_prove_uses_adopter_profile_default_floor(tmp_path: Path) -> None:
    ...
    node_ids = [node['id'] for node in payload['data']['action_graph']['nodes']]
    assert set(node_ids) == {'repository-audit', 'claims', 'evidence-freshness', 'docs-topology', 'schemas', 'playbooks-v2', 'generated-artifacts', 'format-policy', 'asset-determinism', 'schema-contracts', 'proof-policy'}
...
def test_prove_execute_can_select_real_gates(monkeypatch, tmp_path: Path) -> None:
    ...
    gate = GateDescriptor(id='isolated-proof', kind='contract', command=('isolated-proof',), evidence_class='contract', trust_bearing=True)
    ...
    class PassingRunner:
        ...
        def run(self, node, *, root: Path) -> ActionRunResult:
            ...
            return ActionRunResult(action_id=node.id, command=node.command, state='passed', exit_code=0)
    monkeypatch.setattr(proof_cli.git, 'current_head', lambda _root: 'a' * 40)
    monkeypatch.setattr(proof_cli.status_domain, 'audit_for_root', lambda *_args, **_kwargs: {'ok': True, 'required_gaps': [], 'governance_context': {}})
    ...
    monkeypatch.setattr(proof_cli, 'openspec_governance_report', lambda *_args, **_kwargs: {'ok': True, 'required_gaps': []})
    monkeypatch.setattr(proof_cli, 'gate_registry', lambda _root: {gate.id: gate})
    monkeypatch.setattr(proof_cli, 'gate_graph', lambda *_args, **_kwargs: ActionGraph(nodes=(gate.to_node(),)))
...
def test_prove_execute_preserves_non_trust_bearing_gate_classification(monkeypatch) -> None:
    diagnostic_gate = GateDescriptor(id='diagnostic-only', kind='lint', command=('diagnostic', '--check'), evidence_class='diagnostic', trust_bearing=False)
    ...
    monkeypatch.setattr(proof_cli, 'gate_registry', fake_gate_registry)
    monkeypatch.setattr(proof_cli, 'gate_graph', fake_gate_graph)
    ...
    class PassingDiagnosticRunner:
        ...
        def run(self, node, *, root):
            ...
            return ActionRunResult(action_id=node.id, command=node.command, state='passed', exit_code=0, stdout='', stderr='')
    monkeypatch.setattr(proof_cli, 'LocalSubprocessRunner', PassingDiagnosticRunner)
    payload = run_ethos_blocked('prove', '--execute', '--gate', 'diagnostic-only', '--json')
    assert payload['ok'] is False
    assert payload['state'] == 'gapped'
...
def test_adopt_gitlab_profile_is_available(tmp_path: Path) -> None:
    payload = run_ethos('adopt', '--root', str(tmp_path), '--profile', 'gitlab', '--dry-run', '--json')
    assert payload['ok'] is True
...
def test_prove_returns_evidence_and_provenance() -> None:
    ...
    assert payload['data']['governance_context'] == payload['data']['repository_audit']['governance_context']
    assert payload['data']['governance_context']['single_kernel'] is True
    assert payload['data']['provenance']['subject'][0]['digest']['sha256'] == payload['data']['evidence']['digest']
...
def test_prove_uses_repository_audit_for_non_product_repo(tmp_path: Path) -> None:
    ...
    assert payload['data']['repository_audit']['governance_context']['contract'] == 'governed_repository'
    ...
    assert payload['data']['repository_audit']['governance_context']['subject']['kind'] == 'repository'
    assert payload['data']['repository_audit']['governance_context']['subject']['id'] == str(tmp_path.resolve())
    assert payload['data']['repository_audit']['governance_context']['shared_commands'] == ['ethos status', 'ethos plan', 'ethos prove', 'ethos land', 'ethos publish']
    assert payload['data']['repository_audit']['governance_context']['transition_commands'] == ['ethos status', 'ethos plan', 'ethos prove', 'ethos land', 'ethos publish']
    assert payload['data']['repository_audit']['governance_context']['reader_view_commands'] == ['ethos orient']
    assert payload['data']['repository_audit']['governance_context']['scorecard_commands'] == ['ethos report']
...
def test_campaign_status_reports_manifest_steps() -> None:
    ...
    campaign = next(item for item in payload['data']['campaigns'] if item['id'] == 'terminal-openspec-productization')
...
def test_campaign_closeout_scopes_to_selected_campaign() -> None:
    payload = run_ethos('campaign', 'closeout', '--campaign', 'terminal-openspec-productization', '--json')
...
def test_campaign_closeout_reports_local_campaign_packages() -> None:
    ...
    payload = run_ethos('campaign', 'closeout', '--adopter', 'generic', '--target', target.as_posix(), '--json')
    assert payload['command'] == 'campaign closeout'
    assert payload['state'] in {'local_ready', 'gapped'}
    assert payload['summary']['remote_state'] == 'deferred'
    assert payload['summary']['parity_pending_count'] == len(payload['data']['parity']['pending_packages'])
    ...
    assert remote_publication['reason'] in {'remote publication adapter unavailable', 'remote unavailable; use local-ci fallback evidence'}
    ...
    assert set(packages) == {'local_closeout', 'trust_closeout', 'campaign', 'intake_projection', 'publication', 'release', 'parity', 'shadow_parity'}
    ...
    assert packages['local_closeout']['required_gaps'] == payload['data']['workspace']['closeout_support']['required_gaps']
    ...
    if payload['state'] == 'gapped' and (not payload['data']['parity']['required_gaps']):
        assert any(package.get('required_gaps') for package in packages.values() if isinstance(package, dict))
    ...
    assert payload['data']['provenance']['shadow_parity'] == packages['shadow_parity']['provenance']
    assert payload['data']['provenance']['closeout'] == {'mode': 'local_only', 'remote_state': 'deferred'}
