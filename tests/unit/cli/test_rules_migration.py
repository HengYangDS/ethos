...
def test_rules_migrate_dry_run_reports_lossless_target_without_writing(tmp_path: Path) -> None:
...
def test_rules_migrate_apply_requires_authorization(tmp_path: Path, monkeypatch) -> None:
    lane = start_adopted_work_lane(tmp_path)
    path = _write_legacy_rules(lane.worktree)
    monkeypatch.setenv('ETHOS_ACTOR', 'agent:test:case:agent-test')
    payload = run_ethos_blocked('rules', 'migrate', '--root', lane.worktree.as_posix(), '--apply', '--json', cwd=lane.worktree)
...
def test_rules_migrate_apply_rejects_head_mismatch(tmp_path: Path, monkeypatch) -> None:
    ...
    path = _write_legacy_rules(lane.worktree)
    monkeypatch.setenv('ETHOS_ACTOR', 'agent:test:case:agent-test')
    payload = run_ethos_blocked('rules', 'migrate', '--root', lane.worktree.as_posix(), '--apply', '--authorize', '--expect-head', '0' * 40, '--json', cwd=lane.worktree)
...
def test_rules_migrate_apply_rejects_protected_root(tmp_path: Path) -> None:
    ...
    payload = run_ethos_blocked('rules', 'migrate', '--root', repo.as_posix(), '--apply', '--authorize', '--expect-head', head, '--json', cwd=repo)
    assert 'protected_lane_prewrite_blocked' in payload['required_gaps']
    assert path.read_text(encoding='utf-8') == LEGACY_RULES

def test_rules_migrate_apply_writes_only_after_work_lane_admission(tmp_path: Path, monkeypatch) -> None:
    lane = start_adopted_work_lane(tmp_path)
    path = _write_legacy_rules(lane.worktree)
    head = git(lane.worktree, 'rev-parse', 'HEAD')
    monkeypatch.setenv('ETHOS_ACTOR', 'agent:test:case:agent-test')
    payload = run_ethos('rules', 'migrate', '--root', lane.worktree.as_posix(), '--apply', '--authorize', '--expect-head', head, '--json', cwd=lane.worktree)
    assert payload['ok'] is True
...
def test_rules_migrate_apply_rechecks_head_after_prewrite(tmp_path: Path, monkeypatch) -> None:
    lane = start_adopted_work_lane(tmp_path)
    path = _write_legacy_rules(lane.worktree)
    head = git(lane.worktree, 'rev-parse', 'HEAD')
    observed_heads = iter((head, 'f' * 40))
    monkeypatch.setenv('ETHOS_ACTOR', 'agent:test:case:agent-test')
    monkeypatch.setattr(rules_surface.git_adapter, 'current_head', lambda _root: next(observed_heads))
    payload = run_ethos_blocked('rules', 'migrate', '--root', lane.worktree.as_posix(), '--apply', '--authorize', '--expect-head', head, '--json', cwd=lane.worktree)
    assert payload['required_gaps'] == ['expect_head_mismatch']
    assert path.read_text(encoding='utf-8') == LEGACY_RULES

def test_rules_migrate_apply_rechecks_head_inside_serialized_migration(tmp_path: Path, monkeypatch) -> None:
    lane = start_adopted_work_lane(tmp_path)
    path = _write_legacy_rules(lane.worktree)
    head = git(lane.worktree, 'rev-parse', 'HEAD')
...
def test_plan_reports_invalid_rules_profiles_as_top_level_gap(tmp_path: Path) -> None:
    ...
    assert payload['data']['rule_validation_gaps'] == ['rules_profile_invalid:active_must_be_string_array']
