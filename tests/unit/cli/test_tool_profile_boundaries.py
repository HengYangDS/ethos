...
def test_tool_profiles_are_derived_from_the_adoption_catalog() -> None:
    ...
    for entry in catalog:
        ...
        assert adapter["adoption"] == entry["adoption"]
        assert adapter["config"] == entry["config"]
        assert adapter.get("gate") == entry.get("gate")
        assert "planned" not in adapter


def test_tool_catalog_schema_requires_a_declared_adoption_state() -> None:
