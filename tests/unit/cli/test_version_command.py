...
def test_version_json_is_one_utf8_result_not_double_encoded() -> None:
    ...
    assert identity["product_version"] == "0.2.0-alpha.4"
    assert identity["distribution_version"].startswith("0.2.0a4.dev0+")
    assert payload["data"]["openspec_version"] == "1.12.0"
...
def test_version_human_output_is_concise(tmp_path, monkeypatch) -> None:
    ...
    assert completed.returncode == 0, completed.stderr
    assert completed.stdout.startswith("ethos 0.2.0-alpha.4 ")
    assert "0.2.0a4.dev0+" in completed.stdout
    assert "OpenSpec 1.12.0" in completed.stdout
    ...
    assert version_module.version_text().startswith("ethos 0.2.0-alpha.4 ")
