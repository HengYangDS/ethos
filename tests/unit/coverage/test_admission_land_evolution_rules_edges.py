...
def status(role: str = ROLE_WORK_LANE, dirty: bool = False, changed: list[str] | None = None) -> dict[str, object]:
    return {
        "role": role,
        "dirty": dirty,
        "branch": "work/x" if role == ROLE_WORK_LANE else "dev",
        "changed_paths": changed or [],
        "candidate": {"exists": True, "worktree_exists": True, "worktree_path": "/tmp/candidate", "head": "c1"},
...
def test_admission_hook_layers_and_postwrite_fuse(monkeypatch, tmp_path: Path) -> None:
    ...
    assert admission.hook_admission_report(root=tmp_path, layer="context", expected_root=tmp_path)["state"] == "refreshed"
    assert admission.hook_admission_report(root=tmp_path, layer="context", expected_root=tmp_path / "other")["required_gaps"] == ["hook_context_root_mismatch"]
    ...
    risky = admission.hook_admission_report(root=tmp_path, layer="pre-run", command="python -c 'open(1, \"w\")'")
    assert risky["required_gaps"] == ["hook_prerun_paths_required"]

    monkeypatch.setattr(admission, "workspace_status", lambda repo: status(ROLE_ACCEPTED_ROOT, changed=["README.md"]))
    protected = admission.hook_admission_report(root=tmp_path, layer="post-write", paths=[Path("README.md")])
    assert protected["state"] == "fused"
    assert protected["required_gaps"] == ["post_write_protected_root_dirty"]

    monkeypatch.setattr(admission, "workspace_status", lambda repo: status(ROLE_WORK_LANE, changed=["unexpected.md"]))
    unexpected = admission.hook_admission_report(root=tmp_path, layer="post-write", paths=[Path("README.md")])
...
def test_push_and_ref_move_admission(monkeypatch, tmp_path: Path) -> None:
    policy = SimpleNamespace(accepted_branch="dev", role_for_branch=lambda branch: ROLE_ACCEPTED_ROOT if branch == "dev" else ROLE_WORK_LANE, candidate_branch="candidate/dev")
    monkeypatch.setattr("ethos_core.contracts.branch_roles.load_branch_role_policy", lambda root: policy)
    monkeypatch.setattr("ethos.adapters.mutation.core._proof_gaps", lambda root, head: ["proof_not_proven"])
    blocked = admission.push_admission_report(root=tmp_path, target_ref="refs/heads/dev", pushed_head="h1")
    assert blocked["required_gaps"] == ["proof_not_proven"]
    allowed = admission.push_admission_report(root=tmp_path, target_ref="refs/heads/work/x", pushed_head="h1")
    assert allowed["ok"] is True

    monkeypatch.setattr("subprocess.run", lambda *args, **kwargs: cp(returncode=1))
    ref = admission.ref_move_admission_report(root=tmp_path, ref_name="refs/heads/dev", old_value="0" * 40, new_value="h2")
    assert "accepted_advance_not_candidate_validated" in ref["required_gaps"]
    noop = admission.ref_move_admission_report(root=tmp_path, ref_name="refs/heads/dev", old_value="h2", new_value="h2")
...
def test_evolution_ledger_campaign_and_candidate_edges(tmp_path: Path) -> None:
    ...
    assert evolution.campaign_report(tmp_path, campaign_id="missing")["required_gaps"] == ["campaign_missing:missing"]
...
def test_land_readiness_projection_edges(monkeypatch, tmp_path: Path) -> None:
    ...
    assert land.land_next_actions(ok=False, gaps=("candidate_base_stale",), current_head="h1")[0].startswith("ethos lane refresh-base")
    assert land.closeout_next_actions(ok=False, gaps=("candidate_diverged_from_accepted",), current_head="h1")[0].startswith("ethos lane candidate")
    ...
    monkeypatch.setattr(land, "workspace_status", lambda repo: {"candidate": {"worktree_path": str(tmp_path / "candidate")}})
    ...
    assert land.intake_projection_report(tmp_path)["required_gaps"] == ["intake_provider_missing:.ethos/intake.toml"]
    ...
    workspace = {"role": "work_lane", "branch": "work/x", "closeout_support": {"supported": True, "claim_binding": "missing"}}
    ...
    envelope = {"promotion": {"ready": True}, "evidence": {"commands": ["ethos prove --execute --json"]}, "required_gaps": []}
    ready = land.trust_closeout_package(workspace={"role": "work_lane", "branch": "work/x", "closeout_support": {"supported": True, "claim_binding": "bound"}}, claims={"ok": True, "claims": {"c": {"trust_envelope": envelope}}})
...
def test_rules_policy_edge_helpers_and_reports(tmp_path: Path) -> None:
    ...
    assert str(policy_rules.policy_exceptions_report(tmp_path)["required_gaps"][0]).startswith("policy_exception_parse_error:")
