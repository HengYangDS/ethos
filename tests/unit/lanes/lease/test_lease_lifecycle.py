...
def test_same_holder_recovers_legacy_lease_at_current_descendant(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    holder_ref = "agent:test:case:source"
    fixture = start_adopted_work_lane(
        tmp_path / "legacy-recovery", name="legacy-recovery", holder_ref=holder_ref
    )
    branch = "work/legacy-recovery"
    database = state_database(fixture.worktree)
    initial = _lease_snapshot(fixture.worktree, branch)
    change_id = Path(str(initial["base_commitment_path"])).parent.name
    legacy_payload = dict(initial["payload"])
    for field in (
        "expires_at",
        "expected_tree",
    ...
    with closing(sqlite3.connect(database)) as connection, connection:
        connection.execute(
            "update leases set payload_json = ? where subject = ?",
            (json.dumps(legacy_payload, sort_keys=True), branch),
        )
    old_head = str(initial["expected_head"])
    current_head = commit_fixture_file(
        fixture.worktree,
        "forward.txt",
        "forward\n",
        "advance recoverable lane",
    )
    raw = observe_lease(database, branch).record()
    assert raw["lease_state"] == "unknown"
    monkeypatch.setenv("ETHOS_ACTOR", holder_ref)

    dry_run = recover_legacy_lease(
        root=fixture.worktree,
        request=LeaseRecoveryRequest(
            branch=branch,
            holder_ref=holder_ref,
            lease_id=str(initial["lease_id"]),
            expected_epoch=int(initial["epoch"]),
            expect_head=current_head,
            expected_expires_at=str(initial["expires_at"]),
            expected_payload_sha256=str(raw["payload_sha256"]),
            change_id=change_id,
            ttl_seconds=86_400,
            apply=False,
        ),
    )
    assert (dry_run["verdict"], dry_run["state"]) == ("pass", "recoverable")
    assert dry_run["observation"]["legacy_expected_head"] == old_head
    assert observe_lease(database, branch).state == "unknown"

    applied = recover_legacy_lease(
        root=fixture.worktree,
        request=LeaseRecoveryRequest(
            branch=branch,
            holder_ref=holder_ref,
    ...
    assert (applied["verdict"], applied["state"]) == ("pass", "recovered")
    recovered = observe_lease(database, branch)
    assert recovered.state == "valid"
    assert recovered.lease is not None
    assert recovered.lease.expected_head == current_head
    assert recovered.lease.expected_tree == git(fixture.worktree, "rev-parse", "HEAD^{tree}")
    assert recovered.lease.epoch == int(initial["epoch"]) + 1


def test_legacy_lease_recovery_rejects_other_holder_and_diverged_head(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    holder_ref = "agent:test:case:source"
    fixture = start_adopted_work_lane(
        tmp_path / "legacy-recovery-reject", name="legacy-recovery-reject", holder_ref=holder_ref
    )
    branch = "work/legacy-recovery-reject"
    database = state_database(fixture.worktree)
    initial = _lease_snapshot(fixture.worktree, branch)
    payload = dict(initial["payload"])
    payload.pop("expected_tree")
    with closing(sqlite3.connect(database)) as connection, connection:
        connection.execute(
            "update leases set payload_json = ? where subject = ?",
            (json.dumps(payload, sort_keys=True), branch),
        )
    raw = observe_lease(database, branch).record()
    monkeypatch.setenv("ETHOS_ACTOR", "agent:test:case:other")
    rejected = recover_legacy_lease(
        root=fixture.worktree,
        request=LeaseRecoveryRequest(
            branch=branch,
            holder_ref=holder_ref,
            lease_id=str(initial["lease_id"]),
            expected_epoch=int(initial["epoch"]),
            expect_head=str(initial["expected_head"]),
            expected_expires_at=str(initial["expires_at"]),
            expected_payload_sha256=str(raw["payload_sha256"]),
            change_id="legacy-recovery-reject",
            ttl_seconds=86_400,
            apply=True,
        ),
    )
    assert rejected["verdict"] == "block"
    assert rejected["required_gaps"][0] == "lease_actor_mismatch"
    assert observe_lease(database, branch).state == "unknown"


def test_legacy_lease_recovery_rejects_diverged_current_head(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    holder_ref = "agent:test:case:source"
    fixture = start_adopted_work_lane(
        tmp_path / "legacy-recovery-diverged",
        name="legacy-recovery-diverged",
        holder_ref=holder_ref,
    )
    branch = "work/legacy-recovery-diverged"
    database = state_database(fixture.worktree)
    initial = _lease_snapshot(fixture.worktree, branch)
    change_id = Path(str(initial["base_commitment_path"])).parent.name
    legacy_head = str(initial["expected_head"])
    payload = dict(initial["payload"])
    payload.pop("expected_tree")
    with closing(sqlite3.connect(database)) as connection, connection:
        connection.execute(
    ...
    git(fixture.worktree, "checkout", "--detach", f"{legacy_head}^")
    current_head = commit_fixture_file(
        fixture.worktree,
        "diverged.txt",
        "diverged\n",
        "create divergent recovery target",
    )
    raw = observe_lease(database, branch).record()
    monkeypatch.setenv("ETHOS_ACTOR", holder_ref)
    ...
    assert rejected["verdict"] == "block"
    assert "lease_head_not_descendant" in rejected["required_gaps"]
    assert observe_lease(database, branch).state == "unknown"


def test_public_legacy_lease_recovery_dry_run_projects_exact_replacement(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    holder_ref = "agent:test:case:source"
    fixture = start_adopted_work_lane(
        tmp_path / "legacy-recovery-cli", name="legacy-recovery-cli", holder_ref=holder_ref
    )
    branch = "work/legacy-recovery-cli"
    database = state_database(fixture.worktree)
    initial = _lease_snapshot(fixture.worktree, branch)
    payload = dict(initial["payload"])
    payload.pop("expected_tree")
    ...
    result = run_ethos(
        ...
        holder_ref,
        "--lease-id",
        str(initial["lease_id"]),
        "--epoch",
        str(initial["epoch"]),
        "--expect-head",
        str(initial["expected_head"]),
        "--expires-at",
        str(initial["expires_at"]),
        "--payload-sha256",
    ...
    assert (result["verdict"], result["state"]) == ("pass", "recoverable")
    assert result["data"]["replacement"]["expected_head"] == initial["expected_head"]
    assert observe_lease(database, branch).state == "unknown"
