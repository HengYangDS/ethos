...
def _set(payload: dict[str, object], dotted: str, value: object = _DELETE) -> None:
    target = payload
    parts = dotted.split(".")
    for part in parts[:-1]:
...
def test_wcp_response_fails_closed_when_required_binding_is_missing(field: str) -> None:
    ...
    assert (raised.value.gap, raised.value.detail) == (
...
# Native admission is added alongside the predecessor adapter until the later
# effect-reconnection slice removes that adapter and its focused tests.
