...
def test_pre_push_projects_declared_peer_reconciliation_coordinates(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    calls: list[dict[str, object]] = []
    monkeypatch.setattr(hook_commands, "resolve_root", lambda _root: tmp_path)
    monkeypatch.setattr(hook_commands, "emit", lambda *_args, **_kwargs: None)
    monkeypatch.setattr(
        hook_commands,
        "push_admission_report",
        lambda **kwargs: (
    ...
    hook_commands.pre_push(
        ...
        options=hook_commands.PushReconciliationOptions(
            remote_head="",
            remote="origin",
            reconciliation_receipt_path="/receipt.json",
            observed_peer_head=("gitlab=accepted,release",),
            json_output=True,
        ),
    )

    assert calls[0]["reconciliation"].peer_heads == (("gitlab", "accepted", "release"),)
