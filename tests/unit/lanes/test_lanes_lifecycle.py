...
def test_branch_role_policy_semantic_order_uses_configured_roles_without_hardcoded_names() -> None:
    policy = BranchRolePolicy(release_branch='release', accepted_branch='integration', candidate_branch='stage/integration', work_branch_prefix='lane/', submit_branch_prefix='review/')
    status = policy.as_status_policy()
    assert tuple((status[key] for key in status if key != 'semantic_order')) == ('release', 'integration', 'stage/integration', 'lane/', 'review/', 'independent')
    assert [(item['role'], item['kind'], item['config_key'], item['pattern']) for item in status['semantic_order']] == [('release_root', 'exact_branch', 'release_branch', 'release'), ('accepted_root', 'exact_branch', 'accepted_branch', 'integration'), ('candidate', 'exact_branch', 'candidate_branch', 'stage/integration'), ('work_lane', 'branch_prefix', 'work_branch_prefix', 'lane/*'), ('submit_lane', 'branch_prefix', 'submit_branch_prefix', 'review/*')]
    assert [policy.role_for_branch(branch) for branch in ('release', 'integration', 'stage/integration', 'lane/feature', 'review/feature', 'main', 'dev', 'candidate/dev', 'work/feature', 'submit/feature')] == ['release_root', 'accepted_root', 'candidate', 'work_lane', 'submit_lane', 'other', 'other', 'other', 'other', 'other']

def test_start_work_lane_uses_configured_candidate_and_work_role_policy(tmp_path: Path) -> None:
    repo = init_repo(tmp_path / 'repo')
    write_role_policy(repo)
    git(repo, 'worktree', 'add', '-b', 'stage/dev', (tmp_path / 'repo-stage-dev').as_posix(), 'dev')
    worktree = tmp_path / 'repo-lane-feature'
    report = start_work_lane(root=repo, name='feature', path=worktree, holder_ref='agent:test:case:agent-test', apply=True)
    assert report['ok'] is True
    assert report['branch'] == 'lane/feature'
    assert report['base'] == 'stage/dev'
    assert git(worktree, 'branch', '--show-current') == 'lane/feature'

def test_existing_work_lane_claim_binding_can_be_applied_without_restarting_lane(tmp_path: Path) -> None:
    repo = init_repo(tmp_path / 'repo')
    candidate = add_candidate_worktree(repo, tmp_path / 'repo-candidate-dev')
    worktree = tmp_path / 'repo-work-feature'
    start_work_lane(root=repo, name='feature', path=worktree, holder_ref='agent:test:case:agent-test', apply=True)
    ...
    assert tuple((closeout[key] for key in ('supported', 'branch', 'target_branch', 'operation', 'holder_ref', 'lease_epoch', 'claim_id', 'claim_binding', 'required_gaps'))) == (True, 'work/feature', 'candidate/dev', 'land_to_candidate', 'agent:test:case:agent-test', 1, 'sample-trust', 'bound', [])
...
def test_prewrite_rejects_tracked_path_from_accepted_root(tmp_path: Path) -> None:
    repo = init_repo(tmp_path / 'repo')
    report = prewrite_guard(root=repo, paths=[repo / 'README.md'], editor_root=repo, require_editor_root=True)
    assert report['ok'] is False
    assert report['error'] == 'protected_lane_prewrite_blocked'
    assert report['role'] == 'accepted_root'

def test_prewrite_allows_owned_work_lane_with_matching_editor_root(tmp_path: Path, monkeypatch) -> None:
    repo = init_repo(tmp_path / 'repo')
    add_candidate_worktree(repo, tmp_path / 'repo-candidate-dev')
    worktree = tmp_path / 'repo-work-owned'
    start_work_lane(root=repo, name='owned', path=worktree, holder_ref='agent:test:case:agent-test', apply=True)
    monkeypatch.setenv('ETHOS_ACTOR', 'agent:test:case:agent-test')
    monkeypatch.setattr(prewrite, 'workspace_status', pytest.fail, raising=False)
    report = prewrite.prewrite_guard(root=worktree, paths=[worktree / 'README.md'], editor_root=worktree, require_editor_root=True)
    assert (report['ok'], report['role'], report['error']) == (True, 'work_lane', '')

def test_prewrite_blocks_product_root_when_runner_source_differs(tmp_path: Path, monkeypatch) -> None:
    repo = init_repo(tmp_path / 'repo')
    worktree = tmp_path / 'repo-work-owned'
    ...
    monkeypatch.setattr('ethos.adapters.repo.runtime.core.ethos.__file__', external_runner.as_posix())
    report = prewrite_guard(root=worktree, paths=[worktree / 'README.md'], editor_root=worktree, require_editor_root=True)
    assert report['ok'] is False
...
def test_prewrite_rejects_work_lane_without_editor_root_binding(tmp_path: Path, monkeypatch) -> None:
    repo = init_repo(tmp_path / 'repo')
    add_candidate_worktree(repo, tmp_path / 'repo-candidate-dev')
    worktree = tmp_path / 'repo-work-owned'
    start_work_lane(root=repo, name='owned', path=worktree, holder_ref='agent:test:case:agent-test', apply=True)
...
def test_prewrite_rejects_protected_lane_roles(tmp_path: Path) -> None:
    cases = {'release_root': ('main',), 'candidate': ('candidate/dev',), 'submit_lane': ('submit/review',), 'other': ('feature/unknown',)}
...
def test_start_work_lane_apply_creates_worktree_and_records_lease(tmp_path: Path) -> None:
    ...
    assert report['ok'] is True
    assert report['branch'] == 'work/feature'
    assert report['worktree'] == {'branch': 'work/feature', 'path': worktree.resolve().as_posix(), 'head': git(worktree, 'rev-parse', 'HEAD'), 'role': 'work_lane', 'worktree_binding': 'linked'}
    assert worktree.exists()
    assert git(worktree, 'branch', '--show-current') == 'work/feature'
    leases = active_leases(repo / '.ethos' / 'state' / 'state.sqlite')
    assert [(lease['subject'], lease['holder_ref']) for lease in leases] == [('work/feature', 'agent:test:case:agent-test')]
...
def test_start_work_lane_defaults_path_to_sibling_candidate_home(tmp_path: Path) -> None:
    ...
    report = start_work_lane(root=repo, name='feature', holder_ref='agent:test:case:agent-test', apply=True)
    assert report['ok'] is True
    assert report['path'] == expected.resolve().as_posix()
    assert expected.exists()
    assert git(expected, 'branch', '--show-current') == 'work/feature'

@pytest.mark.parametrize(('mode', 'expected_gap'), [('branch_missing', 'candidate_branch_missing'), ('worktree_missing', 'candidate_worktree_missing'), ('worktree_dirty', 'candidate_worktree_dirty')])
def test_start_work_lane_apply_requires_ready_candidate(tmp_path: Path, mode: str, expected_gap: str) -> None:
    repo = init_repo(tmp_path / 'repo')
...
def _stale_work_lane(tmp_path: Path, *, candidate_path: str='CANDIDATE.md', lane_path: str='FEATURE.md', commit_lane: bool=True) -> tuple[Path, Path, Path, str, str]:
    repo = init_repo(tmp_path / 'repo')
    candidate = add_candidate_worktree(repo, tmp_path / 'repo-candidate-dev')
    worktree = tmp_path / 'repo-work-feature'
    ...
    return (repo, candidate, worktree, git(worktree, 'rev-parse', 'HEAD'), git(candidate, 'rev-parse', 'HEAD'))

def _rules(root: Path, records: list[tuple[str, int]]) -> None:
    blocks = ['\n'.join(('[[quality.source_budget.debt.records]]', f'id = "{identifier}"', 'owner = "test"', 'replacement = "test replacement"', 'deletion_wave = "test"', 'expiry = "test"', f'allowance = {allowance}')) for identifier, allowance in records]
    rules = root / '.ethos' / 'rules.toml'
    rules.parent.mkdir(exist_ok=True)
    rules.write_text('\n\n'.join(('[quality.source_budget.debt]', f'maximum_total = {sum((allowance for _identifier, allowance in records))}', *blocks, '[gates.local-state-audit]\ncommand = "test"\nblocking = true', '')), encoding='utf-8')

@pytest.mark.parametrize(('candidate_record', 'lane_record', 'outside_change', 'ok'), [('candidate', 'lane', False, True), ('shared', 'shared', False, False), ('candidate', 'lane', True, False)])
def test_refresh_work_lane_base_handles_source_budget_debt_conflicts(tmp_path: Path, candidate_record: str, lane_record: str, *, outside_change: bool, ok: bool) -> None:
    repo = init_repo(tmp_path / 'repo')
    _rules(repo, [('base', 10)])
    ...
    if outside_change:
        ...
        candidate_rules.write_text(candidate_rules.read_text(encoding='utf-8') + '\n[unrelated]\nvalue = true\n', encoding='utf-8')
    commit_fixture_file(candidate, rules, (candidate / rules).read_text(encoding='utf-8'), 'add candidate debt')
    _rules(worktree, [('base', 10), (lane_record, 30)])
    commit_fixture_file(worktree, rules, (worktree / rules).read_text(encoding='utf-8'), 'add lane debt')
    previous_head = git(worktree, 'rev-parse', 'HEAD')
    candidate_head = git(candidate, 'rev-parse', 'HEAD')
    report = refresh_work_lane_base(root=worktree, apply=True, authorized=True, expect_head=previous_head)
    ...
    assert git(repo, 'merge-base', '--is-ancestor', candidate_head, git(worktree, 'rev-parse', 'HEAD')) == ''
...
def test_ssh_signing_transport_uses_launchd_agent_socket(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    def fake_run(command: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        if command[:2] == ['ssh-add', '-T']:
            return subprocess.CompletedProcess(command, 0 if isinstance(kwargs.get('env'), dict) else 1, '', '')
...
def test_refresh_work_lane_base_blocks_unavailable_file_backed_ssh_before_rebase(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(lane_refresh, 'ssh_signing_transport_ready', lambda _key: False, raising=False)
    monkeypatch.setattr(lane_refresh, 'run_git', signing_git)
    report = lane_refresh.refresh_work_lane_base(root=worktree, apply=True, authorized=True, expect_head=previous_head)
    assert report['required_gaps'] == ['refresh_signing_transport_unavailable']
    assert rebase_calls == []
    assert git(worktree, 'rev-parse', 'HEAD') == previous_head

def test_refresh_work_lane_base_blocks_snapshot_moved_during_preflight(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_refresh_work_lane_base_rechecks_configured_candidate_ref(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(lane_refresh, 'load_branch_role_policy', lambda _root: SimpleNamespace(candidate_branch='stage/dev'))
    monkeypatch.setattr(lane_refresh, 'workspace_status', lambda _root: {'role': 'work_lane', 'dirty': False, 'branch': 'work/feature', 'candidate': {'exists': True, 'worktree_exists': True, 'worktree_path': 'candidate', 'head': 'c1'}})
    ...
    report = lane_refresh.refresh_work_lane_base(root=tmp_path, apply=True, authorized=True, expect_head='h1')
    assert report['required_gaps'] == ['refresh_base_snapshot_stale:candidate']
    assert ('rev-parse', 'stage/dev') in calls

def test_refresh_work_lane_base_does_not_overwrite_branch_moved_before_cas(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    repo, _, worktree, previous_head, candidate_head = _stale_work_lane(tmp_path)
    original_run_git = lane_refresh.run_git

    def move_branch_before_cas(root: Path, *args: str, **kwargs: object) -> subprocess.CompletedProcess[str]:
...
def test_refresh_work_lane_base_rejects_attach_and_post_cas_races(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, attach_code: int, head: str, gap: str) -> None:
    ...
    monkeypatch.setattr(lane_refresh, 'load_branch_role_policy', lambda _root: SimpleNamespace(candidate_branch='candidate/dev'))
    monkeypatch.setattr(lane_refresh, 'workspace_status', lambda _root: {'role': 'work_lane', 'dirty': False, 'branch': 'work/feature', 'candidate': {'exists': True, 'worktree_exists': True, 'worktree_path': 'candidate', 'head': 'candidate'}})
    monkeypatch.setattr(lane_refresh, 'changed_paths', lambda _path: [])
    monkeypatch.setattr(lane_refresh, 'is_ancestor', lambda *_args: next(ancestors))
    monkeypatch.setattr(lane_refresh, 'run_git', run)
    report = lane_refresh.refresh_work_lane_base(root=tmp_path, apply=True, authorized=True, expect_head='work')
...
def test_refresh_work_lane_base_rejects_noop_rebase_success(tmp_path: Path, monkeypatch) -> None:
    ...
    def successful_noop_rebase(root: Path, *args: str, **kwargs: object) -> subprocess.CompletedProcess[str]:
    ...
    assert report['next_actions'] == ['inspect current Git ancestry and runner, signing, or hook diagnostics', 'repair the replay environment and rerun ethos lane refresh-base']

def test_refresh_work_lane_base_apply_requires_authorization_and_expected_head(tmp_path: Path) -> None:
...
def test_start_work_lane_apply_requires_clean_accepted_root(tmp_path: Path, mode: str) -> None:
    ...
    report = start_work_lane(root=root, name=name, path=worktree, holder_ref='agent:test:case:agent-test', apply=True)
    assert report['ok'] is False
    assert report['state'] == 'blocked'
...
@pytest.mark.parametrize(('project_text', 'expected'), [(None, ('external_current_runner', 'workspace_status_runner_source_differs_from_audit_root', 'package-bound runner')), ("command_plane = 'ethos'\n", ('external_current_runner', 'workspace_status_runner_source_differs_from_audit_root', 'package-bound runner')), ("[command_plane]\npublic = 'pixi run ethos'\n", ('external_declared_runner', None, 'declared external runner'))])
def test_runtime_binding_classifies_external_runner_declarations(tmp_path: Path, monkeypatch, project_text: str | None, expected: tuple[str, str | None, str]) -> None:
    repo = init_repo(tmp_path / 'repo')
