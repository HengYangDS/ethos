...
def _create_v1_state(db_path: Path, *, cache_rows: int=0) -> None:
    ...
    with closing(sqlite3.connect(db_path)) as connection:
        ...
        for index in range(cache_rows):
            connection.execute("insert into cache_entries(cache_key, payload_json) values (?, '{}')", (f'cache:{index}',))
        connection.commit()

def test_initialize_state_migrates_empty_cache_without_deleting_existing_state(tmp_path: Path) -> None:
    db_path = tmp_path / '.ethos' / 'state' / 'state.sqlite'
    ...
    with closing(sqlite3.connect(db_path)) as connection:
        ...
        unrelated_state = connection.execute('select id, payload_json from unrelated_state').fetchall()
    assert 'cache_entries' not in tables
    assert versions == [(1,), (2,)]
    assert leases == [('lease:current', 'work/current')]
    assert unrelated_state == [(1, '{}')]

def test_initialize_state_fails_closed_when_retired_cache_is_not_empty(tmp_path: Path) -> None:
    db_path = tmp_path / '.ethos' / 'state' / 'state.sqlite'
...
def test_initialize_state_rolls_back_schema_and_preserves_journal_mode(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    db_path = tmp_path / '.ethos' / 'state' / 'state.sqlite'
    _create_v1_state(db_path)
    with closing(sqlite3.connect(db_path)) as connection:
...
def test_initialize_state_v2_is_idempotent_and_preserves_leases(tmp_path: Path) -> None:
    ...
    initialize_state(db_path)
    with closing(sqlite3.connect(db_path)) as connection:
        assert connection.execute('select version from schema_migrations order by version').fetchall() == [(2,)]
        assert connection.execute('select id, subject from leases').fetchall() == [('lease:current', 'work/current')]
...
def test_acquire_lease_rejects_duplicate_current_lane_incarnation(tmp_path: Path) -> None:
    from ethos.adapters.store.state.lease.lifecycle.core import acquire_lease
    db_path = tmp_path / 'state.sqlite'
    first = acquire_lease(db_path, subject='work/current', holder_ref='agent:codex:thread:first', payload={'lane_incarnation_id': 'lane-incarnation:one', 'expected_head': 'a' * 40})
    with pytest.raises(ValueError, match='lane_lease_conflict'):
        acquire_lease(db_path, subject='work/current', holder_ref='agent:claude:session:second', payload={'lane_incarnation_id': first['payload']['lane_incarnation_id'], 'expected_head': 'a' * 40})

def test_acquire_lease_normalizes_holder_generation_and_timestamps(tmp_path: Path) -> None:
    from ethos.adapters.store.state.lease.lifecycle.core import acquire_lease
    lease = acquire_lease(tmp_path / 'state.sqlite', subject='work/current', holder_ref='agent:codex:thread:first', payload={'expected_head': 'a' * 40, 'claim_id': 'claim-current'})
...
def test_renew_and_resume_require_current_generation_and_same_holder(tmp_path: Path) -> None:
    db_path = tmp_path / 'state.sqlite'
    lease = acquire_lease(db_path, subject='work/current', holder_ref='agent:codex:thread:first', ttl_seconds=60, payload={'expected_head': 'a' * 40})
    renewed = renew_lease(db_path, subject='work/current', holder_ref=lease['holder_ref'], expected_lease_id=lease['lease_id'], expected_epoch=lease['epoch'], expected_head='a' * 40, ttl_seconds=120)
    ...
    with pytest.raises(ValueError, match='lease_epoch_stale'):
        renew_lease(db_path, subject='work/current', holder_ref=lease['holder_ref'], expected_lease_id=lease['lease_id'], expected_epoch=99, expected_head='a' * 40)
    expired_db = tmp_path / 'expired.sqlite'
    expired = acquire_lease(expired_db, subject='work/expired', holder_ref='agent:codex:thread:first', ttl_seconds=-1, payload={'expected_head': 'b' * 40})
    resumed = resume_lease(expired_db, subject='work/expired', holder_ref=expired['holder_ref'], expected_lease_id=expired['lease_id'], expected_epoch=expired['epoch'], expected_head='b' * 40)
...
def test_handoff_offer_accept_changes_holder_and_increments_epoch(tmp_path: Path) -> None:
    db_path = tmp_path / 'state.sqlite'
    lease = acquire_lease(db_path, subject='work/current', holder_ref='agent:codex:thread:first', payload={'expected_head': 'a' * 40})
    offer = offer_lease_handoff(db_path, subject='work/current', holder_ref=lease['holder_ref'], expected_lease_id=lease['lease_id'], expected_epoch=lease['epoch'], target_holder_ref='agent:claude:session:second', expected_head='a' * 40)
    accepted = accept_lease_handoff(db_path, subject='work/current', target_holder_ref='agent:claude:session:second', offer_id=offer['offer_id'], expected_lease_id=lease['lease_id'], expected_epoch=lease['epoch'], expected_head='a' * 40, holder_quiesced=True)
...
def test_handoff_offer_rejects_empty_current_holder(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match='lease_holder_mismatch'):
        offer_lease_handoff(db_path, subject='work/current', holder_ref='', expected_lease_id=lease['lease_id'], expected_epoch=lease['epoch'], target_holder_ref='agent:claude:session:second', expected_head='a' * 40)

def test_revoke_lease_is_generation_and_head_bound(tmp_path: Path) -> None:
    db_path = tmp_path / 'state.sqlite'
    lease = acquire_lease(db_path, subject='work/example', holder_ref='agent:test:case:source', payload={'expected_head': 'a' * 40})
    removed = revoke_lease(db_path, subject='work/example', holder_ref='agent:test:case:source', expected_lease_id=str(lease['lease_id']), expected_epoch=int(lease['epoch']), expected_head='a' * 40)
...
def test_advance_lease_head_is_generation_bound_compare_and_swap(tmp_path: Path) -> None:
    db_path = tmp_path / 'state.sqlite'
    lease = acquire_lease(db_path, subject='work/current', holder_ref='agent:codex:thread:first', payload={'expected_head': 'a' * 40})
    advanced = advance_lease_head(db_path, subject='work/current', holder_ref=lease['holder_ref'], expected_lease_id=lease['lease_id'], expected_epoch=lease['epoch'], old_head='a' * 40, new_head='b' * 40)
    assert advanced['expected_head'] == 'b' * 40
    with pytest.raises(ValueError, match='lease_head_stale'):
        advance_lease_head(db_path, subject='work/current', holder_ref=lease['holder_ref'], expected_lease_id=lease['lease_id'], expected_epoch=lease['epoch'], old_head='a' * 40, new_head='c' * 40)

def test_delete_lease_removes_lease_so_recreated_subject_cannot_inherit(tmp_path: Path) -> None:
    from ethos.adapters.store.state.lease.lifecycle.core import acquire_lease
...
def test_active_leases_reads_through_the_read_only_state_uri(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    db_path = tmp_path / '.ethos' / 'state' / 'state.sqlite'
    lease = state.acquire_lease(db_path, subject='work/feature', holder_ref='agent:test:case:agent-test')
...
def test_active_leases_returns_empty_when_all_sqlite_reads_fail(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    import ethos.adapters.store.state.lease.lifecycle.core as state
    import ethos.adapters.store.state.lease.projection as state_read
    db_path = tmp_path / 'state.sqlite'
