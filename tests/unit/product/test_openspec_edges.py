...
def test_selection_and_validation_helper_edge_cases() -> None:
    ...
    command = {"exit_code": 0, "json": {}, "parse_error": ""}
    assert "openspec_list_failed" in openspec_lifecycle.openspec_command_gaps(
        doctor={**command, "json": {"root": {"healthy": True}}},
        list_result={**command, "exit_code": 1},
        status={},
        validate=command,
        selected=None,
    )
