...
def test_release_topology_rejects_legacy_and_absent_declarations() -> None:
    verbose = {
        "publication": {
            "remote": [
                {
    ...
    compact_list = {"publication": {"remotes": ["origin", "github"]}}

    for declaration in (verbose, compact_list, {}):
        topology = publication_topology(declaration)

        assert topology["state"] == "invalid"
        assert topology["required_gaps"] == ["publication_topology_declaration_invalid"]
...
def test_release_topology_rejects_invalid_and_incomplete_named_declarations() -> None:
    ...
    duplicate = publication_topology(
    ...
    assert incomplete["required_gaps"] == [
        "publication_topology_gitlab_remote_missing",
        "publication_topology_github_remote_missing",
    ]
    assert duplicate["required_gaps"] == ["publication_topology_git_remotes_duplicate"]
