...
exec tools/ci/scripts/with-python-runtime.sh -- uv run --package ethos python tools/ci/ci_templates.py emulator-evidence github --mode "${mode}" ${dry_run_flag:+"${dry_run_flag}"} ${allow_untracked_flag:+"${allow_untracked_flag}"}
