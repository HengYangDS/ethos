...
status_before="${readiness}/status-before.json"
status_after="${readiness}/status-after.json"
mkdir -p "${out}" "${readiness}"
rm -f "${receipt}" "${stderr}" "${status_before}" "${status_after}"
uv run ethos status --json >"${status_before}"
set +e
uv run ethos prove --execute --expect-head "${head}" --json >"${receipt}" 2>"${stderr}"
proof_status=$?
set -e
