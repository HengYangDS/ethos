...
default_ethos_cache="${XDG_CACHE_HOME:-${HOME}/.cache}/ethos/uv"
if [[ "${ETHOS_RUNTIME_BOOTSTRAPPED:-}" != "1" ]]; then
  local_install_uv_cache="${ETHOS_LOCAL_INSTALL_UV_CACHE_DIR:-${UV_CACHE_DIR:-}}"
  if [[ -z "${local_install_uv_cache}" || "${local_install_uv_cache}" == "${default_ethos_cache}" ]]; then
    local_install_uv_cache="$(env -u UV_CACHE_DIR uv cache dir)"
  fi
  exec env ETHOS_LOCAL_INSTALL_UV_CACHE_DIR="${local_install_uv_cache}" \
...
_ethos_finalize_local_install_smoke() {
  ...
  if ! "${script_dir}/require-stable-head.sh" verify \
    "${local_install_head}" \
    "tools/ci/scripts/run-local-install-smoke.sh"; then
    rm -f "${evidence_path}"
    exit 1
  fi
  exit "${exit_code}"
}
trap _ethos_finalize_local_install_smoke EXIT
...
core_wheels=("${artifact_dir}"/ethos_core-*.whl)
if [[ "${#ethos_wheels[@]}" -ne 1 || "${#core_wheels[@]}" -ne 1 ]]; then
  echo "expected exactly one ethos wheel and one ethos-core wheel" >&2
  exit 1
fi

source_python="${ETHOS_PYTHON:-${repo_root}/build/runtime/venv/bin/python}"
env UV_CACHE_DIR="${local_install_uv_cache}" uv venv --offline --clear \
  --python "${source_python}" "${venv_dir}" >&2
smoke_python="${venv_dir}/bin/python"
smoke_ethos="${venv_dir}/bin/ethos"
env UV_CACHE_DIR="${local_install_uv_cache}" uv pip install --offline \
  --python "${smoke_python}" \
  "${core_wheels[0]}" "${ethos_wheels[0]}" >&2
...
origins = {
    "ethos": Path(ethos.__file__).resolve(),
    "ethos_core": Path(ethos_core.__file__).resolve(),
}
if not all(path.is_relative_to(venv) for path in origins.values()):
    raise SystemExit(f"installed package escaped smoke venv: {origins}")
print(json.dumps({name: path.as_posix() for name, path in origins.items()}, sort_keys=True))
PY
)"
"${smoke_ethos}" --help > "${scratch_root}/ethos-help.txt"
version="$("${smoke_ethos}" --version)"
printf '%s\n' "${version}" > "${scratch_root}/ethos-version.txt"

ETHOS_LOCAL_INSTALL_ROOT="${repo_root}" \
ETHOS_LOCAL_INSTALL_HEAD="${local_install_head}" \
ETHOS_LOCAL_INSTALL_EVIDENCE="${evidence_path}" \
ETHOS_LOCAL_INSTALL_ORIGINS="${origins_json}" \
ETHOS_LOCAL_INSTALL_VERSION="${version}" \
ETHOS_LOCAL_INSTALL_ETHOS_WHEEL="${ethos_wheels[0]}" \
ETHOS_LOCAL_INSTALL_CORE_WHEEL="${core_wheels[0]}" \
"${smoke_python}" - <<'PY'
from __future__ import annotations
...
wheels = []
for value in (
    os.environ["ETHOS_LOCAL_INSTALL_CORE_WHEEL"],
    os.environ["ETHOS_LOCAL_INSTALL_ETHOS_WHEEL"],
):
    path = Path(value)
    wheels.append(
        {
...
payload = {
    ...
    "module_origins": json.loads(os.environ["ETHOS_LOCAL_INSTALL_ORIGINS"]),
