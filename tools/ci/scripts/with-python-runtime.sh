...
if [[ "${UV_CACHE_DIR}" != /* ]]; then
  cache_owner_root="${repo_root}"
  if [[ -n "${inherited_runtime_root}" && "${inherited_runtime_root}" != "${repo_root}" ]]; then cache_owner_root="${inherited_runtime_root}"; fi
  export UV_CACHE_DIR="${cache_owner_root}/${UV_CACHE_DIR}"
fi
  ...
  for ((argument_index = 2; argument_index < ${#runtime_command[@]}; argument_index++)); do
    ...
    if [[ "${runtime_command[argument_index]}" == "env" ]]; then
      for ((environment_index = argument_index + 1; environment_index < ${#runtime_command[@]}; environment_index++)); do
        ...
        if [[ "${environment_assignment}" != *=* ]]; then break; fi
        if [[ "${environment_assignment}" == "ETHOS_RUNTIME_BOOTSTRAPPED=1" ]]; then owner_script_handoff="true"; fi
      done
    fi
  done
